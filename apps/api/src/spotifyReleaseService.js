...
async function connection(){
  const result=await getPool().query(`select provider,account_id,account_name,access_token_ciphertext,refresh_token_ciphertext,token_expires_at,scope,connected_at,updated_at from spotify_connections where provider=$1`,[PROVIDER])
  return result.rows[0]||null
}

async function source(){
  const result=await getPool().query(`select source_key,provider,playlist_id,playlist_name,playlist_url,playlist_snapshot_id,normalized_items,status,last_synced_at,last_attempt_at,last_error_code,last_error_message,retry_after_seconds,updated_at from spotify_release_sources where source_key=$1`,[SOURCE_KEY])
  return result.rows[0]||null
}
...
const publicSource=row=>({
  ...
  error:row?.last_error_code?{code:row.last_error_code,message:row.last_error_message||'',retryAfterSeconds:row.retry_after_seconds||null}:null,
})
...
async function validAccessToken(row){
  ...
  try{payload=await tokenRequest({grant_type:'refresh_token',refresh_token:refreshToken})}
    ...
    if(error instanceof HttpError&&error.code==='SPOTIFY_API_ERROR')throw new HttpError(401,'A conexão Spotify precisa ser autorizada novamente.','SPOTIFY_REAUTH_REQUIRED')
    throw error
  }
...
async function performSync(){
  ...
  try{
    ...
    const playlistId=String(meta.id||current.playlist_id)
    const playlistName=String(meta.name||'')
    const playlistUrl=String(meta.external_urls?.spotify||'')
...
export const spotifyReleaseService={
  async adminState(){
    const [conn,row]=await Promise.all([connection(),source()])
    return {
      configured:Boolean(process.env.SPOTIFY_CLIENT_ID&&process.env.SPOTIFY_CLIENT_SECRET&&process.env.SPOTIFY_REDIRECT_URI&&process.env.SPOTIFY_TOKEN_ENCRYPTION_KEY),
      connected:Boolean(conn),
      account:conn?{id:conn.account_id,name:conn.account_name,connectedAt:conn.connected_at,scope:conn.scope}:null,
      source:publicSource(row),
      items:Array.isArray(row?.normalized_items)?row.normalized_items:[],
    }
  },
  async publicState(){
    const row=await source()
    return {source:publicSource(row),items:Array.isArray(row?.normalized_items)?row.normalized_items:[]}
  },
  async createAuthorizationUrl(returnTo){
    ...
    const cleanReturn=safeReturnTo(returnTo)
  ...
  async completeAuthorization({code,state}){
    ...
    const payload=await tokenRequest({grant_type:'authorization_code',code:String(code||''),redirect_uri:redirectUri})
    ...
    const meResponse=await spotifyFetch(`${API_BASE}/me`,{token:accessToken})
    ...
    await getPool().query(`update spotify_release_sources set status=case when playlist_id is null then 'empty' else status end,last_error_code=null,last_error_message=null,retry_after_seconds=null where source_key=$1`,[SOURCE_KEY])
    return safeReturnTo(result.rows[0].return_to)
  },
  async disconnect(){
    await getPool().query(`delete from spotify_connections where provider=$1`,[PROVIDER])
    await getPool().query(`update spotify_release_sources set status='disconnected',last_error_code=null,last_error_message=null,retry_after_seconds=null where source_key=$1`,[SOURCE_KEY])
    return this.adminState()
  },
  async setPlaylist(value){
    if(!await connection())throw new HttpError(409,'Conecte uma conta Spotify antes de vincular a playlist.','SPOTIFY_DISCONNECTED')
    const playlistId=parseSpotifyPlaylistId(value)
    await getPool().query(`update spotify_release_sources set playlist_id=$2,playlist_name='',playlist_url='',playlist_snapshot_id='',status='syncing',last_error_code=null,last_error_message=null,retry_after_seconds=null where source_key=$1`,[SOURCE_KEY,playlistId])
    await performSync()
    return this.adminState()
  },
  async sync(){await performSync();return this.adminState()},
  async syncIfStale(){
    const ttl=Math.max(MIN_SYNC_TTL_MINUTES,Number(process.env.SPOTIFY_RELEASE_SYNC_TTL_MINUTES)||30)
    const claimed=await getPool().query(`update spotify_release_sources set last_attempt_at=now() where source_key=$1 and playlist_id is not null and status<>'disconnected' and (last_attempt_at is null or last_attempt_at<now()-($2::text||' minutes')::interval) returning source_key`,[SOURCE_KEY,String(ttl)])
