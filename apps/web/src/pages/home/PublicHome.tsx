...
function PublicidadeLateralSection(){
  ...
  const imageUrl=configured.imageUrl||legacy.imageUrl,description=configured.description||legacy.bodyLines.filter(Boolean).join(' ')
  const imageFit=configured.adImageFit==='cover'?'cover':'contain'
  const media=imageUrl?<img className="pl-home-sidebar-ad-image" src={imageUrl} alt={legacy.imageAlt||'Publicidade'} style={{objectFit:imageFit}}/>:<img className="pl-home-sidebar-ad-image pl-home-sidebar-ad-fallback" src={portalLogo} alt="Portal Lander" style={{objectFit:'contain'}}/>
  return <section className="pl-home-sidebar-ad official-publicidade-lateral pl-home-configurable-ad" aria-label="Publicidade Lateral" style={{...advertisingResponsiveCssVariables(configured),textAlign:configured.textAlign}}><div className="pl-home-sidebar-ad-inner" style={{background:configured.background||legacy.background,color:configured.textColor||legacy.textColor,border:`1px solid ${legacy.borderColor}`,borderRadius:legacy.radius,overflow:'hidden'}}><AdvertisingAreaLink config={configured} className="pl-home-ad-area-link">{media}<div className="pl-home-sidebar-ad-copy">{configured.title&&<span className="pl-home-sidebar-ad-kicker" style={{color:configured.textColor}}>{configured.title}</span>}{configured.eyebrow&&<h3 style={{color:configured.textColor}}>{configured.eyebrow}</h3>}{description&&<p style={{color:configured.accentColor}}>{description}</p>}{configured.linkLabel&&configured.adLinkEnabled&&configured.linkUrl&&<span className="pl-home-sidebar-ad-link-label" style={{borderColor:configured.accentColor,color:configured.accentColor}}>{configured.linkLabel} →</span>}</div></AdvertisingAreaLink></div></section>
}
...
function HomeContent(){return <main className="pl-main public-shell official-home-sections" aria-label="Seções da Página Inicial"><div className="official-home-top-grid"><EmDestaqueSection/><MaisLidasSection/></div><div className="official-home-middle-grid"><UltimasNoticiasSection/><aside className="official-home-sidebar-stack" aria-label="Coluna lateral da Página Inicial"><PublicidadeLateralSection/><EmAltaSection/></aside></div><AnuncieAquiSection/><div className="official-home-bottom-grid"><LancamentosSection/><AgendaSection/></div></main>}
