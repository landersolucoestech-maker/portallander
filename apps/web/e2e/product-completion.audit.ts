...
async function proveAppearance(page:Page,formId:string,screenshotName:string){
  ...
  const layoutGroup=appearance.locator('details').filter({has:appearance.getByText('Layout e container',{exact:true})})
  await layoutGroup.getByText('Largura relativa (%)',{exact:true}).locator('..').locator('input').fill('82')
  await expect.poll(()=>cssVariable(preview,'--form-container-width')).toBe('82%')
  await layoutGroup.getByText('Raio',{exact:true}).locator('..').locator('input').fill('24')
  await expect.poll(()=>cssVariable(preview,'--form-container-radius')).toBe('24px')
  await layoutGroup.getByText('Colunas',{exact:true}).locator('..').locator('select').selectOption('2')
  await expect.poll(()=>cssVariable(preview,'--form-columns')).toBe('2')

  const buttonGroup=appearance.locator('details').filter({has:appearance.getByText('Botão',{exact:true})})
  await buttonGroup.locator('summary').click()
  const buttonBackground=buttonGroup.getByText('Fundo',{exact:true}).locator('..').locator('input[type="color"]')
  await setColorInput(buttonBackground,'#b80000')
  await expect.poll(()=>cssVariable(preview,'--form-button-bg')).toBe('#b80000')
  await buttonGroup.getByText('Raio',{exact:true}).locator('..').locator('input').fill('18')
  await expect.poll(()=>cssVariable(preview,'--form-button-radius')).toBe('18px')
  const buttonText=buttonGroup.getByText('Texto do botão',{exact:true}).locator('..').locator('input')
...
test.describe('Portal Lander product completion',()=>{
  ...
  test('Marketing exposes Métricas in canonical order, navigates and never falls back to mock Analytics',async({page})=>{
    ...
    await expect(unavailable.or(metricStrip)).toBeVisible()
