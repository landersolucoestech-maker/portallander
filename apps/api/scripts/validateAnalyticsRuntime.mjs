...
try{
  ...
  console.log(JSON.stringify({serviceIdempotency:{rawCount:counts.raw_count,normalizedCount:counts.metric_count,updatedValue:secondMetric.value},accountIsolation:true,zeroPreserved:true,unavailableAsMissing:true,ga4Simulation:{processed:simulatedFirst.processed,rawCountAfterRetry:simCounts.raw_count,normalizedCountAfterRetry:simCounts.normalized_count,emptyReport:emptyResult.sync.status,malformedPayload:malformedRun.status,databaseFailure:dbFailureRun.status}}))
}finally{
  await pool.end()
}
