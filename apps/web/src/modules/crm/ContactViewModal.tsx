...
export function ContactViewModal({open,contact,onClose}:{open:boolean;contact:Contact|null;onClose:()=>void}){
 ...
 return <div className="crm-modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}><section ref={dialogRef} tabIndex={-1} className="crm-modal crm-modal-lg crm-readonly-view" role="dialog" aria-modal="true" aria-label="Visualizar contato"><header className="crm-modal-head crm-readonly-head"><div><div className="crm-badge-row"><span className="crm-badge">{label(contactCategoryOptions,contact.category)}</span><span className="crm-badge muted">{contact.profile}</span><span className="crm-badge muted">{contact.status==='ativo'?'Ativo':'Inativo'}</span><span className="crm-badge muted">{label(priorityOptions,contact.priority)}</span></div><h2>{contact.name}</h2><p>{contact.company||(contact.entityType==='pessoa_fisica'?'Pessoa Física':'Pessoa Jurídica')} · Visualização completa do contato</p></div><button className="crm-icon-btn" onClick={onClose} aria-label="Fechar"><X size={18}/></button></header>
 <div className="crm-modal-body" style={viewStyles.body}>
  <section aria-label="Resumo do contato" style={viewStyles.summary}>
   <Summary label="Tipo" value={contact.entityType==='pessoa_fisica'?'Pessoa Física':'Pessoa Jurídica'}/><Summary label="Categoria" value={label(contactCategoryOptions,contact.category)}/><Summary label="Status" value={contact.status==='ativo'?'Ativo':'Inativo'}/><Summary label="Prioridade" value={label(priorityOptions,contact.priority)}/>
  </section>
  <div style={viewStyles.grid}>
   <Section title="Tipo e classificação"><Data label="Tipo de contato" value={contact.entityType==='pessoa_fisica'?'Pessoa Física':'Pessoa Jurídica'}/><Data label="Categoria" value={label(contactCategoryOptions,contact.category)}/><Data label="Perfil" value={contact.profile}/></Section>
   <Section title="Identificação"><Data label={semantic.name} value={contact.name}/><Data label={semantic.document} value={contact.document}/><Data label={semantic.company} value={contact.company}/></Section>
   <Section title="Contato"><Data label="Email" value={contact.email}/><Data label="Telefone" value={contact.phone}/><Data label="WhatsApp" value={contact.whatsapp}/></Section>
   ...
   <Section title="Localização"><Data label="Cidade" value={contact.city}/><Data label="Estado" value={contact.state}/></Section>
   <Section title="Relacionamento"><Data label="Prioridade" value={label(priorityOptions,contact.priority)}/><Data label="Status" value={contact.status==='ativo'?'Ativo':'Inativo'}/><Data label="Tags" value={contact.tags.join(', ')}/>{contact.sourceLeadId&&<><Data label="Origem" value="Convertido de Lead"/><Data label="ID do lead de origem" value={contact.sourceLeadId}/></>}</Section>
   <Section title="Observações" wide><ReadOnlyBlock label="Observações">{contact.notes||'Nenhuma observação registrada.'}</ReadOnlyBlock></Section>
