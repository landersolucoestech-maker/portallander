...
import {homeReadModel,type HomeAgendaItem,type HomeStory} from '../../../pages/home/models/homeReadModel'
...
type GenericHomeContentSectionId=Exclude<HomeContentSectionId,'lancamentos'>
type PreviewItem={key:string;title:string}

const meta:Record<GenericHomeContentSectionId,{name:string;summary:string;criterion:string;responsive:string}>={
...
function availableFor(sectionId:GenericHomeContentSectionId,config?:HomeContentSectionConfiguration):PreviewItem[]{
...
export function HomeContentSectionPage({sectionId}:{sectionId:GenericHomeContentSectionId}){
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:`Configurar seção: ${info.name}`,description:`Configure ${info.name} no painel rolável à esquerda e acompanhe a Página Inicial completa no preview fixo à direita.`,backTo:'/app/site/paginas',backLabel:'Páginas'}}>
    {loading&&<AdminNotice title={`Sincronizando ${info.name}`} description="Carregando a configuração persistida antes de abrir o editor."/>}{error&&<AdminNotice title="Falha na configuração" description={error}/>} 
    {!loading&&<div className="section-editor-workbench"><div className="section-editor-rail" aria-label={`Configurações da seção ${info.name}`}><SectionEditorSummaryCard eyebrow={info.name.toUpperCase()} description={info.summary} active={config.active} onActiveChange={active=>patch({active})}/><div className="section-editor-tabs" aria-label="Grupos de configuração"><Tab active={tab==='content'} label="Conteúdo" onClick={()=>setTab('content')}/><Tab active={tab==='appearance'} label="Aparência" onClick={()=>setTab('appearance')}/><Tab active={tab==='behavior'} label="Comportamento" onClick={()=>setTab('behavior')}/></div><section className="section-editor-card section-editor-detail">
