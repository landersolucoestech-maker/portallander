...
function NewContractModal({onClose,onCreate}:{onClose:()=>void;onCreate:(contract:Omit<ContractRecord,'id'>)=>void}){
  ...
  const submit=(event:FormEvent)=>{
    ...
    onCreate({title,type:draft.type,status:draft.status,signature:draft.signature,value:numericValue})
  }
  return <div className="contracts-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
    <section className="contracts-modal" role="dialog" aria-modal="true" aria-label="Novo contrato">
      <header><div><h2>Novo contrato</h2><p>Crie um rascunho contratual para a sessão atual.</p></div><button type="button" className="zip-icon" onClick={onClose} aria-label="Fechar"><X size={16}/></button></header>
      <div className="contracts-modal-body"><form className="contracts-form" onSubmit={submit}>
        <label className="wide"><span>Título do contrato</span><input value={draft.title} onChange={event=>setDraft(current=>({...current,title:event.target.value}))} placeholder="Ex.: Pacote de mídia · Empresa" required/></label>
        <label><span>Categoria</span><select value={draft.type} onChange={event=>setDraft(current=>({...current,type:event.target.value}))}>{CATEGORIES.map(category=><option key={category}>{category}</option>)}</select></label>
        <label><span>Status inicial</span><select value={draft.status} onChange={event=>setDraft(current=>({...current,status:event.target.value}))}><option>Rascunho</option><option>Em análise</option><option>Aguardando assinatura</option></select></label>
        <label><span>Assinatura</span><select value={draft.signature} onChange={event=>setDraft(current=>({...current,signature:event.target.value}))}><option>—</option><option>Autentique</option><option>Clicksign</option></select></label>
        <label><span>Valor</span><input value={draft.value} onChange={event=>setDraft(current=>({...current,value:event.target.value}))} placeholder="R$ 0,00" inputMode="decimal"/></label>
        <div className="contracts-form-actions wide"><button type="button" className="button outline" onClick={onClose}>Cancelar</button><button type="submit" className="button dark">Criar rascunho</button></div>
      </form><p className="contracts-session-note">O rascunho é adicionado à tabela da sessão atual. Persistência definitiva depende de backend.</p></div>
    </section>
  </div>
}
...
export function ContractsPage(){
  ...
  const headerActions:readonly AdminShellAction[]=[
    {label:'Templates',variant:'secondary',onClick:()=>navigate('/app/crm/contracts/templates')},
    {label:'Categorias',variant:'secondary',onClick:()=>navigate('/app/crm/contracts/categorias')},
    {label:'Variáveis',variant:'secondary',onClick:()=>navigate('/app/crm/contracts/variaveis')},
    {label:'Novo contrato',variant:'primary',onClick:()=>setShowNew(true)},
  ]
  const createContract=(contract:Omit<ContractRecord,'id'>)=>{
    setContracts(items=>[...items,{id:Math.max(...items.map(item=>item.id),0)+1,...contract}])
    setQuery('');setTypeFilter('Todos');setStatusFilter('Todos');setSort('recent');setPage(1);setShowNew(false)
  }
  ...
  return <AdminShell area="crm" items={CRM_NAV} header={{title:'Contratos',description:'Gestão de documentos, vigência, assinatura e valores contratuais.'}} headerActions={headerActions}>
    <div className="zip-stack contracts-page">
      ...
      <div className="zip-toolbar contracts-toolbar"><label className="zip-search"><Search size={14}/><input value={query} onChange={event=>{setQuery(event.target.value);setPage(1)}} placeholder="Buscar por parceiro, tipo ou título…"/></label><select value={typeFilter} onChange={event=>{setTypeFilter(event.target.value);setPage(1)}}><option>Todos</option>{types.map(type=><option key={type}>{type}</option>)}</select><select value={statusFilter} onChange={event=>{setStatusFilter(event.target.value);setPage(1)}}><option>Todos</option>{statuses.map(status=><option key={status}>{status}</option>)}</select><select value={sort} onChange={event=>setSort(event.target.value)} aria-label="Ordenar contratos"><option value="recent">Mais recentes</option><option value="title">Título A–Z</option><option value="value-desc">Maior valor</option><option value="value-asc">Menor valor</option></select></div>
    ...
    {showNew&&<NewContractModal onClose={()=>setShowNew(false)} onCreate={createContract}/>} 
  </AdminShell>
}
