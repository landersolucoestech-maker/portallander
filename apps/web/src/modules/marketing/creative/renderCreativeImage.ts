...
export async function renderCreativeImage(creative:Parameters<typeof normalizeNewsCreative>[0],format:CreativeFormat){
 ...
 const square=format.aspectRatio>=.8,headerRatio=square?0.42:0.34,headerHeight=Math.round(canvas.height*headerRatio),mediaY=headerHeight,mediaHeight=canvas.height-mediaY
