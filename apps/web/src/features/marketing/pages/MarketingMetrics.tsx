...
export function MarketingMetrics({state}:{state:MarketingSeed}){
  ...
  const ranked=state.contents.map(content=>{
    ...
    const value=safeAggregate(related,'reach')
  ...
  return <>
    ...
    <section className="marketing-card marketing-ranking marketing-ranking-exact"><header><h3>{provider==='all'?'Top Conteúdos':`Ranking · ${provider}`}</h3><p>Ranking somente quando a métrica possui vínculo real com o conteúdo e pode ser agregada sem perda semântica.</p></header><div className="marketing-card-body">{ranked.length?<><div className="marketing-ranking-header"><span>Posição</span><span>Conteúdo</span><span>Contexto/Campanha</span><span>Resultado</span><span>Plataforma</span></div><div className="marketing-ranking-list marketing-content-ranking">{ranked.slice(0,8).map((item,index)=><article key={item.content.id}><span className="marketing-rank">{index+1}</span><div className="marketing-ranking-main"><strong>{item.content.title}</strong><small>{item.content.type}</small></div><div className="marketing-ranking-main"><strong>{item.content.subject||item.content.campaign||item.content.context||'—'}</strong></div><div><strong>{compact(item.value??0)}</strong><small>Alcance</small></div><div><strong>{item.provider||'—'}</strong></div></article>)}</div></>:<Empty empty text="UNAVAILABLE — não existe vínculo analítico e agregação semântica comprovados para conteúdo neste período."/>}</div></section>
  </>
}
