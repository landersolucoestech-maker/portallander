...
import { HeroTickerSettings } from './HeroTickerSettings'
import {
  applyArticleToSlide,
...
export function HeroEditor() {
  ...
  const resetCurrentDevice = () => {
    ...
    updateSlide({ title })
    setDraft(current => ({ ...current, ticker: { ...current.ticker, responsive: { ...(current.ticker.responsive || {}), [viewport]: {} } } }))
    markDirty()
  }
  ...
  const tickerHasOverrides = viewport !== 'desktop' && Object.keys(draft.ticker.responsive?.[viewport] || {}).length > 0
  const deviceHasOverrides = viewport !== 'desktop' && (hasHeroAppearanceOverride(appearance, viewport) || hasSlideVisualOverride(slide, viewport) || slide.title.some(segment => Object.keys(segment.responsive?.[viewport] || {}).length > 0) || tickerHasOverrides)
    ...
    <div className="hero-cms-layout">
      <main className="hero-cms-panel">
        ...
        {tab === 'content' && <>
          ...
          <Accordion id="ticker" title="Barra “AGORA”" description="Ticker editorial contínuo integrado diretamente ao Hero" open={openAccordion === 'ticker'} onToggle={toggleAccordion}>
            <HeroTickerSettings ticker={draft.ticker} viewport={viewport} onChange={ticker => patchConfig({ ticker })} />
          </Accordion>
