...
import {uid,type Employee,type EmployeeDocument,type LeaveEntry,type PayrollEntry,type HrSeed} from './domain'
const STORAGE_KEY='portal-lander:hr:v1'
const EVENT='portal-lander:hr:changed'
const clone=<T>(value:T):T=>structuredClone(value)
const seed=():HrSeed=>getRuntimeDataProvider().hr.seed()
const read=():HrSeed=>{try{const raw=localStorage.getItem(STORAGE_KEY);return raw?JSON.parse(raw) as HrSeed:seed()}catch{return seed()}}
const write=(state:HrSeed)=>{localStorage.setItem(STORAGE_KEY,JSON.stringify(state));window.dispatchEvent(new CustomEvent(EVENT));return clone(state)}
...
export const hrRepository={
 ...
 setLeaveStatus(id:string,status:LeaveEntry['status']){const state=read(),now=stamp();state.leaves=state.leaves.map(x=>x.id===id?{...x,status,approvedBy:status==='approved'?'Admin Portal':x.approvedBy,updatedAt:now}:x);return write(state)},
