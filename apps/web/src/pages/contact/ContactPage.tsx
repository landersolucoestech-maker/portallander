...
export function ContactPage({page}:{page:EditorialPage}){
  ...
  const hero=useSectionConfiguration(page.id,'contact-hero','Hero de Contato')
  const channelsSection=useSectionConfiguration(page.id,'contact-channels','Canais Oficiais')
  ...
  const formDefinition=getSiteFormBySlug('lead-capture')
  ...
  const submit=async({payload,acceptedConsentIds,files,antiSpam}:SiteFormSubmitPayload)=>{
    ...
    await submitSiteForm(activeForm.slug,{payload,acceptedConsentIds,files,antiSpam,source:{entryContext:'contact',page:window.location.pathname,referrer:document.referrer||undefined}})
  ...
  return <PageShell className="contato-page contact-page institutional-page" newsletterConfiguration={newsletter}>
