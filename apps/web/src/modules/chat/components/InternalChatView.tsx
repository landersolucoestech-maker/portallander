...
import {useCreateInternalConversation,useSendInternalMessage} from '../hooks'
...
export default function InternalChatView({conversations,members,messages,onChange}:Props){
 ...
 const createConversation=useCreateInternalConversation(),sendMessage=useSendInternalMessage()
 ...
 const send=()=>{if(!selected||!draft.trim()||sendMessage.isPending)return;void sendMessage.mutateAsync({id:selected.id,body:draft.trim()}).then(()=>{changeDraft('');onChange()})}
 const mutationError=createConversation.error||sendMessage.error
