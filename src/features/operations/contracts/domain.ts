...
export function createNextVersion(contract:Contract):Contract{
 ...
 const next:ContractVersion={...current,id,versionNumber,status:'draft',createdAt:now,createdBy:'Administrador local',approvedAt:undefined,approvedBy:undefined,signedAt:undefined,supersededAt:undefined,rejectionReason:undefined,parties:current.parties.map(p=>({...p,id:`${id}-${p.id}`,versionId:id})),economicRules:current.economicRules.map(r=>({...r,id:`${id}-${r.id}`,versionId:id,status:'draft'}))}
