...
async function assertDashboardHierarchy(page:Page){
 ...
 for(const region of [kpis,analytics,recent,leads,featured,pending])await expect(region).toBeVisible()
 ...
 if(positions.viewport<=900){
  expect(positions.recent).toBeGreaterThan(positions.analytics)
  expect(positions.leads).toBeGreaterThanOrEqual(positions.recent-2)
  expect(positions.featured).toBeGreaterThanOrEqual(positions.leads-2)
  expect(positions.pending).toBeGreaterThanOrEqual(positions.featured-2)
 }else{
  expect(Math.abs(positions.recent-positions.analytics)).toBeLessThanOrEqual(8)
  expect(positions.leads).toBeGreaterThan(positions.analytics)
  expect(Math.abs(positions.featured-positions.leads)).toBeLessThanOrEqual(8)
  expect(Math.abs(positions.pending-positions.leads)).toBeLessThanOrEqual(8)
 }
}
