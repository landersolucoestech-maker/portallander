...
export function HomeContentSectionPage({sectionId}:{sectionId:HomeContentSectionId}){
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:`Configurar seção: ${info.name}`,description:'Edite uma seção por vez e visualize a Página Inicial inteira usando o mesmo renderer público.',backTo:'/app/site/paginas',backLabel:'Páginas'}} headerAction={{label:'Ver no site',icon:ExternalLink,variant:'secondary',onClick:()=>window.open(publicUrl,'_blank','noopener,noreferrer')}}>
    <AdminNotice title="Preview canônico da página inteira" description={`${info.responsive}. Desktop, Tablet e Mobile alteram apenas a largura real do preview; o conteúdo continua único.`}/>
    ...
    <div className="section-config-workbench">
      <aside className="section-config-panel">
        ...
        <div className="section-config-fields">
          ...
          <div className="section-config-responsive-padding"><strong>Comportamento responsivo</strong><p>{info.responsive}. A estrutura é a mesma da Home pública e responde ao viewport real do iframe.</p></div>
