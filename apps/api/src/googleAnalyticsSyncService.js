...
export const googleAnalyticsSyncService={
  async syncRange({startDate,endDate}={}){
    const config=googleAnalyticsConfig(),range=normalizeGa4Range({startDate,endDate})
    if(!config.configured)throw new HttpError(503,'GA4 ainda não possui configuração completa no backend.','GA4_NOT_CONFIGURED')
    const sync=await analyticsService.beginSync({provider:'google-analytics',providerAccountId:config.accountId,providerPropertyId:config.propertyId,scopeType:'portal',scopeId:'portal',metadata:{kind:'ga4-daily-report',range,timezone:config.timezone}})
    try{
      const report=await googleAnalyticsProvider.runDailyReport({startDate,endDate})
      let processed=0
      for(const row of report.rows){
        ...
        for(let index=0;index<GA4_METRIC_MAPPINGS.length;index+=1){
          ...
          const raw=await analyticsService.upsertRawMetric({syncId:sync.id,provider:report.provider,providerAccountId:report.accountId,providerPropertyId:report.propertyId,scopeType:'portal',scopeId:'portal',providerMetric:mapping.providerMetric,value,unit:mapping.unit,periodStart:period.periodStart,periodEnd:period.periodEnd,granularity:'day',timezone:report.timezone,dimensions:{date},filters:{},sourceReference,collectedAt,providerPayload:{dimensionValues:row.dimensionValues,metricValue:row.metricValues?.[index]}})
      ...
      const finished=await analyticsService.finishSync(sync.id,{status:'succeeded',recordsImported:processed,recordsUpdated:0,checkpoint:{endDate:report.range.endDate},cursor:{}})
      ...
      await analyticsService.finishSync(sync.id,{status:'failed',recordsImported:0,recordsUpdated:0,error:error instanceof Error?error.message:'Falha de sincronização GA4.'}).catch(()=>undefined)
      throw error
    }
  },
}
