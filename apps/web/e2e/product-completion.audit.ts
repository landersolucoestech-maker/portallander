...
test.describe('Portal Lander product completion',()=>{
  ...
  test('Métricas stays top-level on the canonical technical route and never falls back to mock Analytics',async({page})=>{
  ...
  test('Dashboard follows the approved reference composition with multichannel performance',async({page})=>{
    ...
    await expect(featured.getByRole('link',{name:/Ver conteúdos/i})).toHaveAttribute('href','#/app/site/content')
    await assertNoHorizontalOverflow(page)
