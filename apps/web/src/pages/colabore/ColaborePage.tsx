  ...
  return <div className="public-page colabore-page">
    ...
    <main>
      ...
      <section className="public-shell colabore-content">
        <div className="colabore-guidelines"><div className="colabore-guidelines-copy"><span>ANTES DE ENVIAR</span><h2>O QUE PROCURAMOS</h2><p>Conteúdo relevante para funk, rap, trap, cultura urbana, entretenimento, bastidores, lançamentos e acontecimentos da cena.</p></div><div className="colabore-guideline"><b>01</b><span>Informação clara e verificável</span></div><div className="colabore-guideline"><b>02</b><span>Material original ou com fonte identificada</span></div><div className="colabore-guideline"><b>03</b><span>Contexto suficiente para análise editorial</span></div></div>
        <div className="colabore-layout"><form className="colabore-form" onSubmit={submit}>
          ...
          <div className="colabore-field-grid colabore-title-type-grid"><label>Título<input required name="titulo" placeholder="Resuma o assunto em uma frase"/></label><label>Assunto / Tipo de conteúdo<div className={`colabore-type-select${typeOpen?' open':''}`}><button type="button" className={`colabore-type-trigger${type?' has-value':''}`} aria-haspopup="listbox" aria-expanded={typeOpen} onClick={()=>setTypeOpen(open=>!open)}><span>{type?submissionLabels[type]:'Selecione o tipo de conteúdo'}</span><i aria-hidden="true"/></button>{typeOpen&&<div className="colabore-type-menu" role="listbox" aria-label="Assunto / Tipo de conteúdo">{(Object.entries(submissionLabels) as [Exclude<SubmissionType,''>,string][]).map(([value,label])=><button key={value} type="button" role="option" aria-selected={type===value} className={type===value?'selected':''} onClick={()=>{setType(value);setTypeOpen(false)}}>{label}</button>)}</div>}<input type="hidden" name="tipo" value={type}/></div></label></div>
