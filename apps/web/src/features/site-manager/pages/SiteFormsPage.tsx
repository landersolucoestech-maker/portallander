import {FileInput,Pencil,UsersRound} from 'lucide-react'
...
export function SiteFormsPage(){
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:'Formulários',description:'Defina e publique formulários do site sem misturar a configuração da captura com a operação dos dados recebidos.'}}>
    ...
    <div className="tableview-surface cms-tableview-surface"><section className="table-card"><table><thead><tr><th>Formulário</th><th>Finalidade</th><th>Destino operacional</th><th>Versão</th><th>Campos</th><th>Consentimentos</th><th>Status</th><th>Ações</th></tr></thead><tbody>{siteFormRegistry.map(form=>{const Icon=form.purpose==='editorial_submission'?FileInput:UsersRound;return <tr key={form.id}><td><div className="table-primary"><span className="table-avatar"><Icon size={15} aria-hidden="true"/></span><div><b>{form.name}</b><small>/{form.slug}</small></div></div></td><td>{purposeLabel[form.purpose]}</td><td>{destinationLabel[form.routing.destination]}</td><td>v{form.version}</td><td>{form.fields.length}</td><td>{form.consents.length}</td><td>{statusLabel[form.status]}</td><td><Link className="button outline" to={`/app/site/formularios/${form.id}`}><Pencil size={14}/>Editar</Link></td></tr>})}</tbody></table></section></div>
    <AdminNotice title="Persistência de produção" description="As respostas não são armazenadas no navegador. O frontend utiliza um contrato de submissão por API; falta conectar a persistência do Portal Lander antes de habilitar recebimento persistente e publicação administrativa."/>
  </AdminShell>
}
