...
export async function handleMarketingRequest(req,res){
  ...
  if(!path.startsWith('/api/marketing/contents'))return false
  const cors=corsHeaders(req,{methods:'GET,POST,PUT,PATCH,DELETE,OPTIONS'})
  if(handleOptions(req,res,{methods:'GET,POST,PUT,PATCH,DELETE,OPTIONS'}))return true
  try{
    ...
    if(match){
      ...
      if(req.method==='GET'){sendJson(res,200,{content:await marketingService.getContent(id)},cors);return true}
      if(req.method==='PATCH'||req.method==='PUT'){const body=await readJson(req);sendJson(res,200,{content:await marketingService.updateContent(id,body.patch??body,body.expectedUpdatedAt,userId)},cors);return true}
      if(req.method==='DELETE'){await marketingService.removeContent(id);sendJson(res,200,{deleted:true,id},cors);return true}
    }
    throw new HttpError(404,'Rota de conteúdo de Marketing não encontrada.','MARKETING_ROUTE_NOT_FOUND')
  }catch(error){
    const status=error instanceof HttpError?error.status:500,code=error instanceof HttpError?error.code:'INTERNAL_ERROR',message=error instanceof HttpError?error.message:'Erro interno da API de Marketing.'
