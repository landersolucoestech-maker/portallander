...
function AdvertisingPreview({config,viewport,name}:{config:AdvertisingSectionConfiguration;viewport:PreviewViewport;name:string}){
  ...
  const image=hasImage?<img className="home-advertising-preview-image" src={config.imageUrl} alt={config.adImageAlt||`Preview ${name}`} style={{objectFit:imageFit,height:'auto'}}/>:null
