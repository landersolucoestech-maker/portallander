...
export function CollaboratePage({page}:{page:EditorialPage}){
  ...
  const hero=useSectionConfiguration(page.id,'collaborate-hero','Hero Colabore')
  const guidelinesSection=useSectionConfiguration(page.id,'collaborate-guidelines','Diretrizes de Envio')
  const formSection=useSectionConfiguration(page.id,'collaborate-form','Formulário de Colaboração')
  ...
  const submit=async({payload,acceptedConsentIds,files,antiSpam}:SiteFormSubmitPayload)=>{
    ...
    await submitSiteForm(activeForm.slug,{payload,acceptedConsentIds,files,antiSpam,source:{entryContext:'collaborate',page:window.location.pathname,referrer:document.referrer||undefined}})
    return activeForm.successMessage
  }

  return <PageShell className="collaborate-page colabore-page special-page" newsletterConfiguration={newsletter}>
    ...
    <main>
      <PageSection><PageContainer>
        ...
        {guidelinesSection.active&&<section className="collaborate-guidelines" style={{background:guidelinesSection.background,color:guidelinesSection.textColor,textAlign:guidelinesSection.textAlign}}><SectionHeading eyebrow={guidelinesSection.eyebrow||'ANTES DE ENVIAR'} title={guidelinesSection.title||'O QUE PROCURAMOS'} description={guidelinesSection.description}/>{guidelines.map(item=><div className="collaborate-guideline" key={item.id}><b>{String(item.order).padStart(2,'0')}</b><span>{item.title}</span></div>)}</section>}
        {formSection.active&&<section className="collaborate-layout" style={{background:formSection.background,color:formSection.textColor,textAlign:formSection.textAlign}}><SectionHeading eyebrow="ENVIO" title={formSection.title||'FORMULÁRIO DE ENVIO'} description={formSection.description}/>{activeForm?<SiteFormRenderer form={activeForm} mode="public" optionSets={resolveSiteFormOptionSets(activeForm)} onSubmit={submit} submitLabel="Enviar material" note="Os materiais enviados são encaminhados ao fluxo de Colaborações recebidas e não são convertidos em Leads do CRM."/>:<div className="editorial-empty-state" role="alert"><h2>Formulário indisponível</h2><p>O formulário Colabore / Anuncie está inativo ou ainda não possui uma versão publicada.</p></div>}</section>}
