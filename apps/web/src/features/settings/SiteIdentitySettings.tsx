...
function FooterIdentityEditor(){
 ...
 return <section className="section-editor-card" aria-labelledby="settings-site-footer-title"><h2 id="settings-site-footer-title">Rodapé global</h2><p>Configuração única aplicada ao final de todas as páginas públicas.</p><label>Exibir identidade visual<select value={config.active?'on':'off'} onChange={event=>patch({active:event.target.value==='on'})}><option value="on">Exibir</option><option value="off">Ocultar</option></select></label><label>Imagem / logo<input value={config.image} onChange={event=>patch({image:event.target.value})}/></label><label>Texto alternativo<input value={config.imageAlt} onChange={event=>patch({imageAlt:event.target.value})}/></label><div className="section-editor-slider"><span>Largura da logo</span><input type="range" min="48" max="360" value={config.width} onChange={event=>patch({width:Number(event.target.value)})}/><b>{config.width}px</b></div><div className="grid-editor-actions" style={{marginTop:20,justifyContent:'flex-start'}}><button type="button" className="button outline" onClick={reset}><RotateCcw size={15}/>Restaurar padrão</button><button type="button" className="button dark" onClick={save}><Save size={15}/>Salvar alterações</button></div>{saved&&<div className="home-section-manager-success grid-save-success">Alterações salvas com sucesso.</div>}</section>
}
