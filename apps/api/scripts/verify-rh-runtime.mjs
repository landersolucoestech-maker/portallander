...
try{
 ...
 const employee=(await hrService.saveEmployee({name:'Runtime Employee',email:'rh@example.com',contractType:'CLT',status:'ativo',baseSalary:5000,department:'Tecnologia'},null,ownerId)).item
 assert.ok(employee.id);assert.equal(employee.baseSalary,5000)
 const payroll=(await hrService.savePayroll({employeeId:employee.id,referenceMonth:'2026-09',grossSalary:5000,discounts:500,bonus:250,status:'processado'},null,ownerId)).item
 assert.equal(payroll.netSalary,4750)
 const leave=(await hrService.saveLeave({employeeId:employee.id,type:'férias',startDate:'2026-10-01',endDate:'2026-10-10',status:'aprovado',approvedBy:'Admin Portal',approvedByDisplayName:'Fake Approver',approvedByUserId:editorId},null,ownerId)).item
