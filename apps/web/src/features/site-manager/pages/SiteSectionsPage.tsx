...
const EDITORIAL_TEMPLATE_SECTION:SiteSection={
  id:'editorial-template',
  name:'Template editorial de Notícias',
  summary:'Layout compartilhado pela listagem e pelas páginas de conteúdo. Alterações estruturais são herdadas por todas as páginas editoriais.',
  kind:'section',
  locked:true,
}
...
export function SiteSectionsPage(){
  ...
  const pages=useMemo<CmsPageOption[]>(()=>{
    const systemPages=editorialReadModel.pages.map(page=>({
      id:page.id,
      title:page.title,
      slug:page.slug,
      source:'system' as const,
      public:isPublicPage(page),
      layout:CUSTOM_LAYOUT_SLUGS.has(page.slug)?'custom' as const:'editorial' as const,
    }))
    return [
      {id:'home',title:'Página inicial',slug:'',source:'system' as const,public:true,layout:'custom' as const},
      ...systemPages,
      ...draftPages.map(page=>({...page,source:'draft' as const,public:false,layout:'editorial' as const})),
    ]
  },[draftPages])
  ...
  const pageSections:SiteSection[]=isEditorialLayout
    ? [EDITORIAL_TEMPLATE_SECTION]
    : [...(selectedPage==='home'?HOME_SECTIONS:[]),...customSections.map(section=>({id:section.id,name:section.name,summary:`Seção própria de ${selected.title}.`,kind:'section' as const}))]
  ...
  const createPage=()=>{
    ...
    setDraftPages(next);sitePageRepository.saveDraftPages(next);setSelectedPage(page.id);setTitle('');setSlug('');setError('');setCreateOpen(false)
  }
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:'Páginas',description:'Gerencie páginas próprias e páginas editoriais sem quebrar a herança visual do site.'}}>
    ...
    {selected.source==='draft'&&<AdminNotice title="Página em rascunho" description="Este rascunho já nasce como página de conteúdo e herda o template de Notícias. Ele ainda não possui versão publicada porque o backend editorial persistente não está conectado."/>}
