...
async function assertRemovedDashboardCards(page:Page){
 await expect(page.getByRole('heading',{name:'Funil Comercial',exact:true})).toHaveCount(0)
 await expect(page.getByTestId('dashboard-lead-distribution')).toHaveCount(0)
...
test('dashboard reproduces the approved executive reference on desktop',async({page})=>{
 for(const viewport of desktopViewports){
  ...
  for(const heading of ['Performance Digital','Hoje & Próximos','Atividades Recentes','Conteúdo & Publicações']){
   await expect(page.getByRole('heading',{name:heading,exact:true})).toBeVisible()
  }
  await assertRemovedDashboardCards(page)
  ...
  for(const region of [performance,agenda,activity,content])await expect(region).toBeVisible()
  ...
  if(viewport.width>1280){
   ...
   expect(performanceBox).not.toBeNull();expect(agendaBox).not.toBeNull();expect(activityBox).not.toBeNull();expect(contentBox).not.toBeNull()
...
test('tablet and mobile preserve reference order without overflow',async({page})=>{
 for(const viewport of compactViewports){
  ...
  expect(await renderedKpiLabels(page)).toEqual(expectedKpis)
  await assertRemovedDashboardCards(page)
  const ids=['dashboard-kpi-region','dashboard-analytics-region','dashboard-today-next','dashboard-recent-activity','dashboard-featured-content']
  const positions=[]
