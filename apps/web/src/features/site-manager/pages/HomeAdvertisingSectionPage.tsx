...
function AdvertisingPreview({config,viewport,name}:{config:AdvertisingSectionConfiguration;viewport:PreviewViewport;name:string}){
  if(!config.active)return <div className="section-preview-disabled">Esta publicidade está desativada. Nenhum espaço é reservado no frontend.</div>
  const layout=advertisingViewportLayout(config,viewport)
  const hasImage=Boolean(config.imageUrl.trim())
  const style={width:layout.width>0?`${layout.width}px`:'100%',height:hasImage?'auto':layout.height>0?`${layout.height}px`:'auto',justifySelf:layout.align==='left'?'start':layout.align==='right'?'end':'center',margin:`${layout.marginY}px ${layout.marginX}px`,padding:hasImage?'0':`${layout.paddingY}px ${layout.paddingX}px`,transform:`translate(${layout.offsetX}px,${layout.offsetY}px)`,background:hasImage?'transparent':config.background,color:config.textColor} as CSSProperties
  const creative=hasImage?<img className="home-advertising-preview-image" src={config.imageUrl} alt={config.adImageAlt||`Preview ${name}`} style={{objectFit:config.adImageFit,height:'auto'}}/>:<div className="home-advertising-preview-placeholder"><div>{config.eyebrow&&<span className="home-advertising-preview-kicker">{config.eyebrow}</span>}<strong>{config.title||name}</strong>{config.description&&<p>{config.description}</p>}{config.linkLabel&&<span className="home-advertising-preview-button">{config.linkLabel} →</span>}</div></div>
  return <div className="home-advertising-preview-stage"><section className={`home-advertising-preview-card ${hasImage?'has-creative':'is-empty'} ad-fit-${config.adImageFit}`} style={style}>{config.adLinkEnabled&&config.linkUrl?<a className="home-advertising-preview-clickable" href={config.linkUrl} target={config.adLinkTarget==='new'?'_blank':'_self'} rel={config.adLinkTarget==='new'?'noreferrer':undefined} onClick={event=>event.preventDefault()} aria-label={`Link da publicidade para ${config.linkUrl}`}>{creative}</a>:creative}</section></div>
}
...
export function HomeAdvertisingSectionPage({sectionId}:{sectionId:AdvertisingSectionId}){
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title,description:'A configuração da publicidade é única. Desktop é a referência; Tablet e Mobile apenas reorganizam e redimensionam a mesma arte.',backTo:'/app/site/paginas',backLabel:'Páginas'}} headerAction={{label:'Ver no site',icon:ExternalLink,variant:'secondary',onClick:()=>window.open(publicUrl,'_blank','noopener,noreferrer')}}>
    <AdminNotice title="Responsividade sem duplicação" description={isSidebar?'No Desktop a publicidade permanece lateral. Em Tablet/Mobile ela entra no fluxo principal, ocupa a largura disponível e mantém a proporção da arte.':'O banner preserva o Desktop e passa a ocupar a largura disponível em Tablet/Mobile sem deformar a arte.'}/>
    ...
    <div className="section-config-workbench">
      <aside className="section-config-panel">
        ...
        <div className="section-config-fields">
          ...
          <div className="home-advertising-device-card"><div className="home-advertising-device-card-head"><strong>Layout Desktop de referência</strong><span>Tablet e Mobile são derivados automaticamente deste componente.</span></div><div className="section-config-two"><Field label={`Largura · ${desktop.width||'100%'}${desktop.width?'px':''}`}><input type="number" min="0" max="1920" value={desktop.width} onChange={event=>patch({adWidthDesktop:clamp(Number(event.target.value)||0,0,1920)})}/></Field><Field label={`Altura sem imagem · ${desktop.height||'automática'}${desktop.height?'px':''}`}><input type="number" min="0" max="1600" value={desktop.height} onChange={event=>patch({adHeightDesktop:clamp(Number(event.target.value)||0,0,1600)})}/></Field></div><Field label="Alinhamento Desktop"><select value={desktop.align} onChange={event=>patch({adAlignDesktop:event.target.value as AdvertisingAlignment})}><option value="left">Esquerda</option><option value="center">Centro</option><option value="right">Direita</option></select></Field></div>
