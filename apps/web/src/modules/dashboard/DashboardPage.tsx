 ...
 return <AdminShell area="crm" items={UNIFIED_ADMIN_NAV} header={{title:'DASHBOARD',description:`Visão Geral · ${formatMonth(data.period.month)}`}}>
  <main className="unified-dashboard" aria-busy={activity.isLoading||analytics.isLoading||adminDashboard.isLoading}>
   <section className="dashboard-kpi-grid" data-testid="dashboard-kpi-region" aria-label="Indicadores principais">
    <article className="dashboard-kpi-card" data-dashboard-kpi="new-leads"><span className="dashboard-kpi-icon"><UsersRound size={21}/></span><div className="dashboard-kpi-copy"><span data-dashboard-kpi-label>Novos Leads</span><div className="dashboard-kpi-value-line"><strong>{data.availability.crm?data.crmSummary.newLeadsThisMonth:'—'}</strong>{trendLabel(leadTrend)&&<span className={`dashboard-trend ${leadTrend!==null&&leadTrend<0?'is-negative':''}`}><TrendingUp size={11}/>{trendLabel(leadTrend)}</span>}</div><small>{data.availability.crm?'em relação ao mês anterior':'Fonte CRM indisponível'}</small></div></article>
    <article className="dashboard-kpi-card" data-dashboard-kpi="active-negotiations"><span className="dashboard-kpi-icon"><Handshake size={21}/></span><div className="dashboard-kpi-copy"><span data-dashboard-kpi-label>Negociações Ativas</span><div className="dashboard-kpi-value-line"><strong>{data.availability.crm?data.crmSummary.activeOpportunities:'—'}</strong></div><small>{data.availability.crm?'oportunidades em andamento':'Fonte CRM indisponível'}</small></div></article>
    <article className="dashboard-kpi-card" data-dashboard-kpi="pipeline-value"><span className="dashboard-kpi-icon"><BarChart3 size={21}/></span><div className="dashboard-kpi-copy"><span data-dashboard-kpi-label>Pipeline Comercial</span><div className="dashboard-kpi-value-line"><strong>{data.availability.crm?money(data.crmSummary.pipelineValue):'—'}</strong></div><small>{data.availability.crm?'em oportunidades':'Fonte CRM indisponível'}</small></div></article>
    <article className="dashboard-kpi-card" data-dashboard-kpi="receivable"><span className="dashboard-kpi-icon"><WalletCards size={21}/></span><div className="dashboard-kpi-copy"><span data-dashboard-kpi-label>A Receber</span><div className="dashboard-kpi-value-line"><strong>{data.availability.finance?money(data.financeSummary.receivable):'—'}</strong></div><small>{data.availability.finance?'em receitas pendentes':'Fonte financeira indisponível'}</small></div></article>
    <article className="dashboard-kpi-card" data-dashboard-kpi="month-revenue"><span className="dashboard-kpi-icon"><CircleDollarSign size={21}/></span><div className="dashboard-kpi-copy"><span data-dashboard-kpi-label>Faturamento do Mês</span><div className="dashboard-kpi-value-line"><strong>{data.availability.finance?money(data.financeSummary.monthRevenue):'—'}</strong>{trendLabel(revenueTrend)&&<span className={`dashboard-trend ${revenueTrend!==null&&revenueTrend<0?'is-negative':''}`}><TrendingUp size={11}/>{trendLabel(revenueTrend)}</span>}</div><small>{data.availability.finance?'em relação ao mês anterior':'Fonte financeira indisponível'}</small></div></article>
   </section>
   ...
   <section className="dashboard-reference-row" aria-label="Performance e agenda">
    <section className="dashboard-reference-panel dashboard-performance-panel" data-testid="dashboard-analytics-region" aria-labelledby="dashboard-performance-title">
     <header className="dashboard-panel-heading"><div className="dashboard-title-with-icon"><span className="dashboard-section-icon"><BarChart3 size={20}/></span><div><h2 id="dashboard-performance-title">Performance Digital</h2><p>Acompanhe o desempenho dos seus canais</p></div></div><Link to="/app/metrics">Ver métricas <ArrowRight size={14}/></Link></header>
     <div className="dashboard-channel-tabs dashboard-reference-tabs" data-testid="dashboard-channel-tabs" role="tablist" aria-label="Canais de performance">{(['overview','instagram','youtube','tiktok','site'] as const).map(tab=><button key={tab} type="button" role="tab" aria-selected={performanceTab===tab} onClick={()=>setPerformanceTab(tab)}>{performanceTabLabels[tab]}</button>)}</div>
     {performanceTab==='overview'?<>
      ...
      <div className="dashboard-performance-body"><div className="dashboard-chart-stage" data-testid="dashboard-website-chart">{chartPoints.length>=2?<><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Evolução recente de pageviews"><polygon points={`0,100 ${chartCoordinates} 100,100`} className="dashboard-chart-area"/><polyline points={chartCoordinates} className="dashboard-chart-line"/></svg><div className="dashboard-chart-labels">{chartPoints.map(point=><span key={point.date}>{point.label}</span>)}</div></>:<div className="dashboard-chart-unavailable"><BarChart3 size={24}/><strong>Série temporal indisponível</strong><p>Os indicadores permanecem sem dados fictícios quando a fonte não responde.</p></div>}</div><aside className="dashboard-channel-distribution" aria-label="Distribuição por canal"><strong>Distribuição por canal</strong>{channels.map(channel=><div className="dashboard-channel-row" key={channel.key}><div><span>{performanceTabLabels[channel.key]}</span><b>{channel.value===null?'—':compact(channel.value)}</b></div><div className="dashboard-channel-track"><i style={{width:`${channel.value===null?0:Math.max(5,(channel.value/maxChannelValue)*100)}%`}}/></div><small>{channel.metricLabel}</small></div>)}</aside></div>
    ...
    <aside className="dashboard-reference-panel dashboard-agenda-panel" data-testid="dashboard-today-next" aria-labelledby="dashboard-agenda-title">
     <header className="dashboard-panel-heading"><div className="dashboard-title-with-icon"><span className="dashboard-section-icon"><CalendarDays size={20}/></span><div><h2 id="dashboard-agenda-title">Hoje & Próximos</h2><p>Compromissos e prioridades operacionais</p></div></div><Link to="/app/agenda">Ver agenda <ArrowRight size={14}/></Link></header>
     ...
     {agendaTab==='agenda'?<div className="dashboard-agenda-list">{todayAgenda.length?todayAgenda.map((event,index)=><article key={event.id} data-dashboard-next-item className={index===0?'dashboard-today-next__item--highlight':undefined}><time dateTime={event.startsAt}><strong>{formatTime(event.startsAt)}</strong><small>{formatDate(event.startsAt)}</small></time><span className="dashboard-agenda-dot"/><div><strong>{event.title}</strong><p>{event.type||event.status}</p></div><span className="dashboard-status-pill">{event.status}</span></article>):<div className="dashboard-empty-inline"><strong>Nenhum compromisso próximo</strong><p>A agenda não possui eventos futuros nesta carga.</p></div>}</div>:<div className="dashboard-task-list">{taskItems.length?taskItems.map((item,index)=><article key={item.id} data-dashboard-next-item className={index===0?'dashboard-today-next__item--highlight':undefined}><CheckSquare size={16}/><div><strong>{item.title}</strong><p>{item.detail}</p></div></article>):<div className="dashboard-empty-inline"><strong>Nenhuma tarefa pendente</strong><p>As fontes disponíveis não indicam ação pendente agora.</p></div>}</div>}
   ...
   <section className="dashboard-bottom-row" aria-label="Atividades e conteúdo">
    ...
    <section className="dashboard-reference-panel dashboard-content-panel" data-testid="dashboard-featured-content" aria-labelledby="dashboard-content-title">
     <header className="dashboard-panel-heading"><div className="dashboard-title-with-icon"><span className="dashboard-section-icon"><FileText size={20}/></span><div><h2 id="dashboard-content-title">Conteúdo & Publicações</h2><p>Gerencie seus conteúdos em um só lugar</p></div></div><Link to="/app/site/content">Criar conteúdo <ArrowRight size={14}/></Link></header>
     <div className="dashboard-content-stats"><div><strong>{data.availability.editorial?data.editorialCounts.publishedThisMonth:'—'}</strong><span><i className="is-green"/>publicados<small>no último mês</small></span></div><div><strong>—</strong><span><i className="is-yellow"/>agendados<small>sem fonte editorial</small></span></div><div><strong>{data.availability.editorial?data.editorialCounts.drafts:'—'}</strong><span><i className="is-gray"/>rascunhos<small>em edição</small></span></div></div>
