...
const viewStyles={
 body:{display:'grid',gap:20,padding:'22px 24px'} as const,
 ...
 grid:{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:'20px 24px'} as const,
 ...
 item:{display:'grid',gridTemplateColumns:'minmax(120px,.7fr) minmax(0,1.3fr)',gap:14,padding:'8px 0',borderBottom:'1px solid #f3f3f3',alignItems:'baseline'} as const,
...
export function LeadViewModal({open,lead,onClose}:{open:boolean;lead:Lead|null;onClose:()=>void}){
 ...
 return <div className="crm-modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}><section ref={dialogRef} tabIndex={-1} className="crm-modal crm-modal-lg crm-readonly-view" role="dialog" aria-modal="true" aria-label="Visualizar lead"><header className="crm-modal-head crm-readonly-head"><div><div className="crm-badge-row"><span className={`crm-badge status-${lead.status}`}>{label(leadStatusOptions,lead.status)}</span><span className="crm-badge muted">{label(priorityOptions,lead.priority)}</span><span className="crm-badge muted">{label(leadTypeOptions,lead.type)}</span></div><h2>{lead.name}</h2><p>{lead.company||'Sem empresa informada'} · Visualização completa do lead</p></div><button className="crm-icon-btn" onClick={onClose} aria-label="Fechar"><X size={18}/></button></header>
 <div className="crm-modal-body" style={viewStyles.body}>
  <section aria-label="Resumo do lead" style={viewStyles.summary}>
   <Summary label="Status" value={label(leadStatusOptions,lead.status)}/><Summary label="Serviço" value={label(serviceOptions,lead.service)}/><Summary label="Temperatura" value={label(temperatureOptions,lead.temperature)}/><Summary label="Valor estimado" value={typeof lead.estimatedValue==='number'?lead.estimatedValue.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'—'}/>
  </section>
  <div style={viewStyles.grid}>
   <Section title="Identificação"><Data label="Nome do contato" value={lead.name}/><Data label="Empresa / Marca" value={lead.company}/><Data label="Cargo / Função" value={lead.role}/></Section>
   <Section title="Contato e presença digital"><Data label="Email" value={lead.email}/><Data label="Telefone / WhatsApp" value={lead.phone}/><Data label="Instagram" value={lead.instagram}/><Data label="Website" value={lead.website}/></Section>
   <Section title="Localização"><Data label="Cidade" value={lead.city}/><Data label="Estado" value={lead.state}/></Section>
   <Section title="Qualificação"><Data label="Tipo de Lead" value={label(leadTypeOptions,lead.type)}/><Data label="Origem" value={label(originOptions,lead.origin)}/><Data label="Prioridade" value={label(priorityOptions,lead.priority)}/><Data label="Temperatura" value={label(temperatureOptions,lead.temperature)}/></Section>
   <Section title="Interesse comercial" wide><Data label="Serviço / Interesse" value={label(serviceOptions,lead.service)}/><ReadOnlyBlock label="Descrição da necessidade">{lead.description||'Nenhuma descrição registrada.'}</ReadOnlyBlock></Section>
   ...
   <Section title="Gestão comercial"><Data label="Status" value={label(leadStatusOptions,lead.status)}/><Data label="Responsável" value={lead.responsible}/><Data label="Campanha de origem" value={lead.campaign}/><Data label="Valor estimado" value={typeof lead.estimatedValue==='number'?lead.estimatedValue.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'—'}/></Section>
   <Section title="Organização"><Data label="Tags" value={lead.tags.join(', ')}/><Data label="Próximo follow-up" value={lead.nextFollowUp?new Date(lead.nextFollowUp).toLocaleString('pt-BR'):'—'}/></Section>
   <Section title="Observações" wide><ReadOnlyBlock label="Observações internas">{lead.notes||'Nenhuma observação interna registrada.'}</ReadOnlyBlock></Section>
   ...
   <Section title="Metadados" wide><Data label="Criado em" value={new Date(lead.createdAt).toLocaleString('pt-BR')}/><Data label="Atualizado em" value={new Date(lead.updatedAt).toLocaleString('pt-BR')}/>{lead.convertedContactId&&<Data label="ID do contato convertido" value={lead.convertedContactId}/>}</Section>
  </div>
...
function ReadOnlyBlock({label,children}:{label:string;children:ReactNode}){return <div style={{padding:'8px 0'}}><dt style={{...viewStyles.term,marginBottom:7}}>{label}</dt><dd style={{margin:0}}><p style={viewStyles.note}>{children}</p></dd></div>}
