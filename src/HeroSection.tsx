...
export function HeroSection({ config, previewIndex = 0, disableAutoplay = false }: { config?: HeroCarouselConfig; previewIndex?: number; disableAutoplay?: boolean }) {
  const [runtimeConfig, setRuntimeConfig] = useState<HeroCarouselConfig>(() => config || readHeroConfig())
  const [activeIndex, setActiveIndex] = useState(previewIndex)
  const [paused, setPaused] = useState(false)

  useEffect(() => {
    if(config){setRuntimeConfig(config);setActiveIndex(Math.max(0,previewIndex));return}
    const sync=()=>{setRuntimeConfig(readHeroConfig());setActiveIndex(0)}
  ...
  return <>
    <section className="portal-hero editorial-hero" aria-label="Destaque principal" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)}>
      ...
      <div className="shell editorial-hero-grid">
        ...
        <div className="editorial-hero-media" style={{['--hero-image-scale' as string]:hero.imageScale,['--hero-image-offset-x' as string]:`${hero.imageOffsetX}px`,['--hero-image-offset-y' as string]:`${hero.imageOffsetY}px`}}>
      ...
      {slides.length>1&&<div className="hero-carousel-controls shell" aria-label="Navegação dos destaques">
        <div className="hero-carousel-arrows">
          <button type="button" aria-label="Destaque anterior" onClick={()=>go(-1)}><ArrowLeft size={17}/></button>
          <button type="button" aria-label="Próximo destaque" onClick={()=>go(1)}><ArrowRight size={17}/></button>
        </div>
    ...
    {runtimeConfig.ticker.active&&<section className="portal-breaking editorial-ticker" aria-label="Agora">
      <SmartLink to={runtimeConfig.ticker.url||'/'} className="shell"><strong>{runtimeConfig.ticker.label}</strong><i aria-hidden="true" /><p>{runtimeConfig.ticker.text}</p><ArrowRight size={20}/></SmartLink>
    </section>}
  </>
}
