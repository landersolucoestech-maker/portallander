...
export function resolveMetricsRange(input={},env=process.env){
  ...
  else throw new HttpError(400,'Período de Métricas inválido.','METRICS_RANGE_INVALID')
...
async function loadConversions(range,pool){
  const {rows}=await pool.query(`
    select f.slug,f.name,f.purpose,
           coalesce(nullif(s.source->>'entryContext',''),'unknown') as entry_context,
           count(*)::int as count,
           count(*) filter (where s.routing_results ? 'crmLeadId')::int as leads_created,
           count(*) filter (where s.routing_results ? 'collaborationId')::int as collaborations_created
      from form_submissions s
      join site_forms f on f.id=s.form_id
     where s.processing_status='accepted'
       and s.submitted_at >= $1::date
       and s.submitted_at < ($2::date + interval '1 day')
     group by f.slug,f.name,f.purpose,coalesce(nullif(s.source->>'entryContext',''),'unknown')
     order by count desc,f.name,entry_context`,[range.startDate,range.endDate])
  const contexts={contato:0,colabore:0,anuncie:0}
  for(const row of rows){if(Object.hasOwn(contexts,row.entry_context))contexts[row.entry_context]+=Number(row.count)||0}
  const forms=rows.map(row=>({slug:row.slug,name:row.name,purpose:row.purpose,entryContext:row.entry_context||'unknown',count:Number(row.count)||0,leadsCreated:Number(row.leads_created)||0,collaborationsCreated:Number(row.collaborations_created)||0}))
  return {status:'available',total:forms.reduce((sum,row)=>sum+row.count,0),leadsCreated:forms.reduce((sum,row)=>sum+row.leadsCreated,0),collaborationsCreated:forms.reduce((sum,row)=>sum+row.collaborationsCreated,0),contexts,forms}
}

export function createMetricsService({ga4Provider=googleAnalyticsProvider,env=process.env,cacheStore=cache,pool=null,poolResolver=getPool}={}){
  return {async overview(input={}){
    const range=resolveMetricsRange(input,env),key=`${range.startDate}:${range.endDate}`,cached=cacheStore.get(key)
