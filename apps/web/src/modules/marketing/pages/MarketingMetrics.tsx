...
export function MarketingMetrics({state,periodStart,periodEnd,previousPeriodStart,previousPeriodEnd,hidePeriodControl=false,providerOverride,embeddedAdmin=false}:MarketingMetricsProps){
  ...
  const base=useMemo(()=>metrics.filter(metric=>activeProvider==='all'||metric.provider===activeProvider),[metrics,activeProvider])
  const previousBase=useMemo(()=>previousMetrics.filter(metric=>activeProvider==='all'||metric.provider===activeProvider),[previousMetrics,activeProvider])
