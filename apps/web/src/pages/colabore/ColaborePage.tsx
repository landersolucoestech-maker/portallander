...
export function ColaborePage(){
  ...
  return <div className="public-page colabore-page">
    <PublicHeader/>
    <main>
      {hero.active&&<section className="colabore-hero public-standard-page-hero" style={{background:heroBackground,backgroundSize:'cover',backgroundPosition:'center',backgroundRepeat:'no-repeat',color:hero.imageUrl?'#fff':hero.textColor,textAlign:hero.textAlign,position:'relative',overflow:'hidden'}}><div className="public-shell colabore-hero-grid" style={{position:'relative',zIndex:1}}><div className="colabore-hero-copy"><span className="colabore-eyebrow" style={{color:hero.accentColor}}>{hero.eyebrow||'PARTICIPE DO PORTAL'}</span><h1>{hero.title||'SUA HISTÓRIA PODE VIRAR NOTÍCIA.'}</h1><p>{hero.description||'Tem uma pauta, vídeo, foto, denúncia, lançamento ou história relevante? Prepare seu material para análise editorial do Portal Lander.'}</p><div className="colabore-hero-points"><span><ShieldCheck size={17}/> Material sujeito à análise da equipe editorial</span><span><CheckCircle2 size={17}/> Envio não garante publicação</span></div></div></div></section>}
