...
export const crmRepository={
 ...
 convertLead(leadId:string):Contact{
  ...
  const email=normalizeText(lead.email),phone=normalizePhone(lead.phone);const existing=state.contacts.find(x=>(email&&normalizeText(x.email)===email)||(phone&&(normalizePhone(x.phone)===phone||normalizePhone(x.whatsapp)===phone)));if(existing){lead.convertedContactId=existing.id;lead.status='fechado';lead.updatedAt=now();write(state);return existing}
