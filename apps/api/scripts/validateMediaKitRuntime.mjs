...
const pool=getPool()
const start='2026-08-01T03:00:00.000Z',end='2026-09-01T03:00:00.000Z'
try{
  ...
  const draftInput={
    status:'published',version:999,
    audience:{
      snapshot:[{id:'forged',value:777777}],snapshotResolvedAt:'2020-01-01T00:00:00Z',
      metrics:[
  ...
  for(const placementId of ['invented-takeover','300x600-random','homepage-mega-takeover'])assert.throws(()=>normalizePayload({inventory:{placements:[{placementId}]}}),error=>error?.code==='MEDIA_KIT_PLACEMENT_NOT_CANONICAL')

  console.log(JSON.stringify({publishedVersion:published.version,authoritativeSnapshotValue:byId.live.value,forgedSnapshotIgnored:true,mockExcludedStatus:byId.mock.dataStatus,manualStatus:byId.manual.dataStatus,immutableAfterAnalyticsUpdate:immutable.value,canonicalPlacements:published.inventory.placements.map(item=>item.placementId)}))
}finally{
  await pool.end()
}
