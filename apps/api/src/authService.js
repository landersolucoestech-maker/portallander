...
function publicUser(row){
  return {id:row.id,email:row.email,displayName:row.display_name||'',role:row.role,lastLoginAt:row.last_login_at||null}
}

async function maybeBootstrapUser(client,email,password){
  const bootstrapEmail=normalizeEmail(process.env.PORTAL_ADMIN_EMAIL)
  const bootstrapPassword=String(process.env.PORTAL_ADMIN_PASSWORD||'')
  if(!bootstrapEmail||!bootstrapPassword||email!==bootstrapEmail||!safeEqualText(password,bootstrapPassword))return null
  const passwordHash=await hashPassword(password)
  const displayName=text(process.env.PORTAL_ADMIN_NAME)||'Administrador'
  const {rows}=await client.query(`
    insert into admin_users(id,email,password_hash,display_name,role,active)
    values($1,$2,$3,$4,'owner',true)
    on conflict do nothing
    returning *`,[`admin_${randomUUID()}`,email,passwordHash,displayName])
  if(rows[0])return rows[0]
  const existing=await client.query('select * from admin_users where lower(email)=lower($1) limit 1',[email])
...
async function recordFailure(client,user){
  ...
  if(attempts>=MAX_FAILED_ATTEMPTS){
    await client.query(`update admin_users set failed_attempts=0,locked_until=now()+($2||' minutes')::interval,updated_at=now() where id=$1`,[user.id,String(LOCK_MINUTES)])
  }else{
    await client.query('update admin_users set failed_attempts=$2,updated_at=now() where id=$1',[user.id,attempts])
  }
}
...
export const authService={
  async login({email,password,remember=false,userAgent='',ipHash=''}){
    ...
    try{
      return await withTransaction(async client=>{
        ...
        if(!user){
          await scrypt(rawPassword,randomBytes(16),PASSWORD_KEY_LENGTH,{N:SCRYPT_N,r:SCRYPT_R,p:SCRYPT_P,maxmem:64*1024*1024})
          throw new HttpError(401,'E-mail ou senha inválidos.','ADMIN_INVALID_CREDENTIALS')
        }
        ...
        if(!valid){await recordFailure(client,user);throw new HttpError(401,'E-mail ou senha inválidos.','ADMIN_INVALID_CREDENTIALS')}

        const token=randomBytes(32).toString('base64url')
        const tokenHash=sha256(token)
        const ttlMs=remember?REMEMBER_SESSION_DAYS*24*60*60*1000:DEFAULT_SESSION_HOURS*60*60*1000
  ...
  async session(token){
    ...
    try{
      ...
      const row=rows[0]
      if(!row)return null
      await getPool().query('update admin_sessions set last_seen_at=now() where id=$1',[row.session_id])
      return {expiresAt:row.expires_at,user:publicUser(row)}
    }catch(error){throw authSchemaError(error)}
  },

  async logout(token){
    if(!token)return
    try{await getPool().query('delete from admin_sessions where token_hash=$1',[sha256(token)])}
    catch(error){throw authSchemaError(error)}
  },

  async revokeUserSessions(userId){
    try{await getPool().query('delete from admin_sessions where user_id=$1',[userId])}
    catch(error){throw authSchemaError(error)}
  },
}
