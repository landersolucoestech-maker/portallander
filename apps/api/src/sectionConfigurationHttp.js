...
import {sectionConfigurationService} from './sectionConfigurationService.js'
...
export async function handleSectionConfigurationRequest(req,res){
  const url=new URL(req.url||'/',`http://${req.headers.host||'localhost'}`)
  const path=url.pathname.replace(/\/+$/,'')||'/'
  const match=path.match(/^\/api\/editorial\/section-configurations\/([^/]+)(?:\/([^/]+))?$/)
  if(!match)return false
  const cors=corsHeaders(req)
  if(req.method==='OPTIONS'){res.writeHead(204,cors);res.end();return true}
  try{
    const pageKey=decodeURIComponent(match[1])
    const sectionSlug=match[2]?decodeURIComponent(match[2]):''
    if(req.method==='GET'){
      const publicOnly=url.searchParams.get('public')==='1'
      if(!publicOnly)await requireAdmin(req)
      if(sectionSlug){
        const configuration=await sectionConfigurationService.get(pageKey,sectionSlug,{publicOnly})
        send(res,200,{configuration},cors);return true
      }
      const configurations=await sectionConfigurationService.list(pageKey,{publicOnly})
      send(res,200,{configurations},cors);return true
    }
    if((req.method==='PUT'||req.method==='PATCH')&&sectionSlug){
      await requireAdmin(req)
      const body=await readJson(req)
      const configuration=await sectionConfigurationService.save(pageKey,sectionSlug,body.configuration)
      send(res,200,{configuration},cors);return true
    }
