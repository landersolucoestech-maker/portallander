...
function AdvertisingAreaLink({config,children,className}:{config:AdvertisingSectionConfiguration;children:ReactNode;className?:string}){if(!config.adLinkEnabled||!config.linkUrl)return <>{children}</>;if(config.adLinkTarget==='same'&&!/^https?:\/\//i.test(config.linkUrl))return <Link to={config.linkUrl} className={className}>{children}</Link>;const href=/^https?:\/\//i.test(config.linkUrl)?config.linkUrl:`${window.location.pathname}#${config.linkUrl}`;return <a href={href} target={config.adLinkTarget==='new'?'_blank':'_self'} rel={config.adLinkTarget==='new'?'noreferrer':undefined} className={className}>{children}</a>}
...
function PublicidadeLateralSection({configuration}:{configuration:SectionConfiguration}){
  const configured=withAdvertisingSectionLayout(configuration,'publicidade-lateral');if(!configured.active)return null
  const imageUrl=configured.imageUrl.trim(),imageFit=configured.adImageFit==='cover'?'cover':'contain',description=configured.description.trim()
