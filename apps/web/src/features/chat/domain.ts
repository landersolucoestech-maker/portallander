...
export type MessageDeliveryStatus='sent'|'failed'|'internal_only'
...
export interface MusicChatMenuOption{id:string;order:number;label:string;responseTemplateId:string;queue:string;sector:string;defaultAssignee?:string|null;tags:string[];priority:MusicChatPriority;active:boolean;requiredFields?:string[];optionalFields?:string[]}
export interface MusicChatTemplate{id:string;title:string;body:string}
export interface MusicChatEscalationRule{id:string;afterMinutes:number;level:string;recipientRole:'supervisor'|'manager'|string;recipientUserId?:string|null;channels:MusicChatNotificationChannel[];active:boolean}
export interface MusicChatAutomationSettings{
 enabled:boolean;welcomeMessage:string;mainMenuMessage:string;menuOptions:MusicChatMenuOption[];templates:MusicChatTemplate[];requiredFields:string[];optionalFields:string[];invalidOptionMessage:string;absenceMessage:string;outOfHoursMessage:string;closingMessage:string;returnToMenuRule:{enabled:boolean;commands:string[]};escalationRules:MusicChatEscalationRule[];notificationChannels:Record<MusicChatNotificationChannel,boolean>;supervisorUserId:string|null;managerUserId:string|null;updatedAt:string
}
export interface ChatSeed{supportConversations:SupportConversation[];supportMessages:SupportMessage[];internalMembers:InternalMember[];internalConversations:InternalConversation[];internalMessages:InternalMessage[];quickReplies:string[];automation:MusicChatAutomationSettings}
