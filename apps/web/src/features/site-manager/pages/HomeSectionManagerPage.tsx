...
export type SectionKey='grid'|'most-read'|'side-ad'|'latest'|'trending'|'horizontal-ad'|'releases'|'agenda'|'footer'
...
const defs:Record<SectionKey,Definition>={
  ...
  'side-ad':{title:'Publicidade lateral',description:'Configuração da publicidade real exibida abaixo de Mais Lidas.',position:'Abaixo de Mais Lidas, lateral direita',identifier:'home_pub_lateral',defaultTitle:'PUBLICIDADE',defaultSubtitle:'ANUNCIE AQUI',defaultQuantity:1,defaultWidth:300,defaultHeight:600,sourceLabel:'Slot',sourceOptions:['HOME_SIDEBAR_01','HOME_SIDEBAR_02']},
  latest:{title:'Últimas Notícias',description:'Configuração da seção real “ÚLTIMAS NOTÍCIAS” exibida na Home.',position:'Abaixo do primeiro bloco editorial; Em Alta fica à direita',identifier:'home_ultimas_noticias',defaultTitle:'ÚLTIMAS NOTÍCIAS',defaultSubtitle:'',defaultQuantity:4,defaultWidth:100,defaultHeight:320,sourceLabel:'Fonte dos conteúdos',sourceOptions:['Últimas notícias','Seleção manual']},
  trending:{title:'Em Alta',description:'Configuração da lista real “EM ALTA” exibida ao lado de Últimas Notícias.',position:'Lateral direita de Últimas Notícias',identifier:'home_em_alta',defaultTitle:'EM ALTA',defaultSubtitle:'',defaultQuantity:4,defaultWidth:300,defaultHeight:360,sourceLabel:'Fonte',sourceOptions:['Em alta','Mais lidas','Seleção manual']},
  'horizontal-ad':{title:'Publicidade horizontal',description:'Configuração da publicidade horizontal real exibida entre Últimas Notícias e Lançamentos.',position:'Entre Últimas Notícias e Lançamentos',identifier:'home_publicidade_horizontal',defaultTitle:defaultHomeAdConfig.title||'',defaultSubtitle:defaultHomeAdConfig.subtitle||'',defaultQuantity:1,defaultWidth:defaultHomeAdConfig.contentWidth,defaultHeight:defaultHomeAdConfig.height,sourceLabel:'Slot',sourceOptions:['HOME_HORIZONTAL']},
  releases:{title:'Lançamentos',description:'Configuração da seção real “LANÇAMENTOS” exibida na Home.',position:'Abaixo da Publicidade horizontal; Agenda fica à direita',identifier:'home_lancamentos',defaultTitle:'LANÇAMENTOS',defaultSubtitle:'',defaultQuantity:4,defaultWidth:100,defaultHeight:420,sourceLabel:'Fonte dos conteúdos',sourceOptions:['Lançamentos','Seleção manual']},
  agenda:{title:'Agenda',description:'Configuração da seção real “AGENDA” exibida ao lado de Lançamentos.',position:'Lateral direita de Lançamentos',identifier:'home_agenda',defaultTitle:'AGENDA',defaultSubtitle:'',defaultQuantity:6,defaultWidth:300,defaultHeight:420,sourceLabel:'Fonte',sourceOptions:['Próximos eventos','Seleção manual']},
  footer:{title:'Rodapé',description:'Configuração do rodapé real que encerra a página inicial.',position:'Encerramento da Home',identifier:'home_footer',defaultTitle:'Portal Lander',defaultSubtitle:'',defaultQuantity:1,defaultWidth:100,defaultHeight:300,sourceLabel:'Estrutura',sourceOptions:['Rodapé atual']},
}

const defaultConfig=(d:Definition):SectionConfig=>({active:true,title:d.defaultTitle,subtitle:d.defaultSubtitle,linkLabel:d.identifier==='home_ultimas_noticias'?'VER TODAS AS NOTÍCIAS':d.identifier==='home_publicidade_horizontal'?(defaultHomeAdConfig.buttonLabel||''):d.identifier==='home_grid_principal'?'EXPLORAR DESTAQUES':'VER TODOS',linkUrl:d.identifier==='home_publicidade_horizontal'?(defaultHomeAdConfig.buttonUrl||'#'):'#',source:d.sourceOptions[0],quantity:d.defaultQuantity,width:d.defaultWidth,height:d.defaultHeight,paddingX:d.identifier==='home_pub_lateral'?0:24,paddingY:d.identifier==='home_pub_lateral'?0:24,radius:0,background:d.identifier==='home_pub_lateral'?'#090909':'#ffffff',textColor:d.identifier==='home_pub_lateral'?'#ffffff':'#333333',titleColor:d.identifier==='home_pub_lateral'?'#ffffff':'#111111',accentColor:'#e50914',borderColor:d.identifier==='home_pub_lateral'?'#090909':'#e5e5e5',bodyLines:d.identifier==='home_pub_lateral'?['SUA MARCA NO RITMO CERTO!']:[],imageUrl:'',imageAlt:'',imageStored:false})
const storageKey=(section:SectionKey)=>section==='side-ad'?SIDEBAR_AD_STORAGE_KEY:`portal-lander:cms:section-config:${section}:v1`
...
function Preview({section,config,items,viewport}:{section:SectionKey;config:SectionConfig;items:PreviewItem[];viewport:PreviewViewport}){
  ...
  if(section==='horizontal-ad')return <div className={`section-preview-stage ${viewport}`}><div className="section-preview" style={shell}>{config.imageUrl&&<img src={config.imageUrl} alt={config.imageAlt} style={{display:'block',width:'100%',height:'100%',objectFit:'cover'}}/>}<div style={{padding:16}}>{config.title&&<h3 style={{color:config.titleColor}}>{config.title}</h3>}{config.subtitle&&<p>{config.subtitle}</p>}{config.linkLabel&&<span style={{display:'inline-block',border:`1px solid ${config.accentColor}`,padding:'8px 12px',color:config.accentColor}}>{config.linkLabel}</span>}</div></div></div>
...
export function HomeSectionManagerPage({section}:{section:SectionKey}){
  ...
  const save=async()=>{if(saving||imageBusy)return;setSaving(true);setSaveError('');try{if(section==='side-ad')await saveSidebarAdConfig(config as SidebarAdConfig);else if(section==='horizontal-ad'){const current=readHomeAdConfig();writeHomeAdConfig({...current,active:config.active,title:config.title,subtitle:config.subtitle,buttonLabel:config.linkLabel,buttonUrl:config.linkUrl,image:config.imageUrl,imageAlt:config.imageAlt,height:config.height,contentWidth:config.width<=100?current.contentWidth:config.width})}else localStorage.setItem(storageKey(section),JSON.stringify(config));setSaved(true);window.dispatchEvent(new CustomEvent(SECTION_UPDATED_EVENT,{detail:{section}}))}catch(error){setSaved(false);setSaveError(error instanceof Error?`Não foi possível salvar: ${error.message}`:'Não foi possível salvar esta seção.')}finally{setSaving(false)}}
  ...
  const handlesImage=section==='side-ad'||section==='horizontal-ad'
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:`Configurar seção: ${d.title}`,description:d.description,backTo:'/app/site/secoes',backLabel:'Seções das Páginas'}} headerAction={{label:'Ver no site',icon:ExternalLink,variant:'secondary',onClick:openPublicSite}}>
    <div className="section-editor-layout grid-editor-layout">
      <section className="section-editor-card grid-editor-settings">
        ...
        {section==='side-ad'&&<><h2>Conteúdo da publicidade</h2><div style={{display:'grid',gap:10}}>{config.bodyLines.map((line,index)=><div key={index} style={{display:'grid',gridTemplateColumns:'1fr auto',gap:8,alignItems:'center'}}><input value={line} onChange={e=>updateBodyLine(index,e.target.value)} aria-label={`Texto ${index+1}`}/><button type="button" className="button outline" onClick={()=>removeBodyLine(index)}><Trash2 size={15}/></button></div>)}</div><button type="button" className="button outline" onClick={addBodyLine} style={{marginTop:10}}><Plus size={15}/> Adicionar texto</button></>}
        ...
        <div className="section-editor-card section-details grid-details-inline"><h2>Detalhes da seção</h2><dl><dt>Identificador</dt><dd>{d.identifier}</dd><dt>Posição na página</dt><dd>{d.position}</dd><dt>Comportamento</dt><dd>Esta configuração representa o mesmo bloco existente na Home, sem seção paralela ou nome legado.</dd><dt>Responsividade</dt><dd>Automática pelo frontend</dd></dl></div>
      </section>

      <section className="section-editor-preview-column grid-editor-preview">
        <div className="section-editor-card section-preview-card"><div className="section-preview-toolbar"><div><h2>Prévia da seção</h2><p>Prévia compacta do mesmo bloco existente na Home.</p></div><div className="hero-cms-viewports" aria-label="Prévia única"><button type="button" className="active" aria-label="Prévia compacta" title="Prévia compacta"><Smartphone size={17}/></button></div></div><Preview section={section} config={config} items={previewItems} viewport="mobile"/></div>
