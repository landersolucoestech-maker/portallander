...
function AdvertisingPreview({config,viewport,name}:{config:AdvertisingSectionConfiguration;viewport:PreviewViewport;name:string}){
  ...
  const image=config.imageUrl?<img className="home-advertising-preview-image" src={config.imageUrl} alt={`Preview ${name}`} style={{objectFit:imageFit}}/>:<div className="home-advertising-preview-placeholder">CARREGUE OU SELECIONE A IMAGEM DO ANÚNCIO</div>
