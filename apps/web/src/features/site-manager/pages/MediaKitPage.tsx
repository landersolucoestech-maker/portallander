...
export function MediaKitPage(){
  const [draft,setDraft]=useState<MediaKitDraft>(()=>mediaKitRepository.read())
  ...
  const patch=(next:MediaKitDraft)=>{setDraft({...next,status:'draft'});setSaved(false)}
  ...
  const saveDraft=()=>{setDraft(mediaKitRepository.save(draft));setSaved(true)}
  const resetDraft=()=>{if(!window.confirm('Descartar o rascunho atual do Mídia Kit e restaurar a estrutura padrão?'))return;setDraft(mediaKitRepository.reset());setSaved(false)}
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:'Mídia Kit',description:'Gerencie a apresentação comercial, audiência, formatos publicitários e canais de contato do Portal Lander.'}}>
    <AdminNotice title="Rascunho administrativo" description="O Mídia Kit pode ser salvo localmente como rascunho para continuidade da edição. Publicar permanece indisponível até existir persistência compartilhada no backend; o navegador nunca transforma este rascunho em conteúdo público."/>
    {saved&&<AdminNotice title="Rascunho salvo" description="As alterações do Mídia Kit foram salvas neste navegador. Nenhuma publicação pública foi realizada."/>}
    ...
    <div className="site-form-editor" style={{marginTop:16}}>
      ...
      <div className="site-form-editor-top" style={{justifyContent:'flex-end'}}><button type="button" className="button outline" onClick={resetDraft}><RotateCcw size={15}/>Restaurar rascunho</button><button type="button" className="button" onClick={saveDraft}><Save size={15}/>Salvar rascunho</button><button type="button" className="button" disabled title="A publicação depende da persistência compartilhada do Portal Lander">Publicar</button></div>
    </div>
  </AdminShell>
}
