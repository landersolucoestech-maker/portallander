...
export function SiteFormsPage(){
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:'Formulários',description:'Defina formulários do site sem misturar configuração, publicação e operação dos dados recebidos.'}} headerAction={{label:'Novo formulário',icon:Plus,onClick:()=>void createForm()}}>
    ...
    <div className="tableview-surface cms-tableview-surface"><section className="table-card"><table className="site-forms-table"><thead><tr><th>Formulário</th><th>Origem</th><th>Finalidade</th><th>Destino operacional</th><th>Versão</th><th>Campos</th><th>Consentimentos</th><th>Status</th><th className="actions-center">Ações</th></tr></thead><tbody>{forms.map(form=>{const Icon=form.purpose==='editorial_submission'?FileInput:UsersRound;const custom=form.source==='custom';return <tr key={form.id}><td><div className="table-primary"><span className="table-avatar"><Icon size={15} aria-hidden="true"/></span><div><b>{form.name}</b><small>/{form.slug}</small></div></div></td><td>{form.source==='system'?'Sistema':persisted?'Personalizado':'Rascunho local'}</td><td>{purposeLabel[form.purpose]}</td><td>{destinationLabel[form.routing.destination]}</td><td>v{form.version}</td><td>{form.fields.length}</td><td>{form.consents.length}</td><td>{statusLabel[form.status]}</td><td className="actions-center"><TableRowActionMenu align="center" label={`${form.name} · ações disponíveis`} onEdit={()=>openForm(form)} onDelete={()=>void deleteDraft(form)} deleteDisabled={!custom} duplicateLabel="Duplicar" onDuplicate={custom?()=>void duplicateForm(form):undefined}/></td></tr>})}</tbody></table></section></div>
