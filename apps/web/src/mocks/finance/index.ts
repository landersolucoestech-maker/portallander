...
export const mockFinanceInvoices:FinanceInvoice[]=Array.from({length:42},(_,index)=>{
 ...
 const amount=outgoing?3600+((index*2100)%32000):650+((index*870)%12500)
 const productsAmount=Math.round(amount*.92*100)/100,freightAmount=Math.round(amount*.02*100)/100,discountAmount=index%4===0?Math.round(amount*.01*100)/100:0
 return {id:`finance_invoice_${String(index+1).padStart(3,'0')}`,number:String(1001+index).padStart(6,'0'),series:outgoing?'001':'E1',type:outgoing?'saida':'entrada',party,document:`${String(12000000000000+index*771133).padStart(14,'0')}`,issueDate,dueDate,amount,status:invoiceStatuses[index%invoiceStatuses.length],description:outgoing?['Publicidade institucional','Produção de conteúdo patrocinado','Prestação de serviços de comunicação'][index%3]:['Infraestrutura de tecnologia','Produção audiovisual','Serviços profissionais'][index%3],model:'55',accessKey:`352608${String(12000000000000+index*771133).padStart(14,'0')}5500100${String(1001+index).padStart(6,'0')}1${String(10000000+index).slice(-8)}`,operationNature:outgoing?'Prestação de serviços / venda':'Aquisição de serviços / entrada',cfop:outgoing?'5933':'1933',entryExitDate:issueDate,stateRegistration:String(110042490114+index),municipalRegistration:String(4800000+index),email:`fiscal${index+1}@${party.toLowerCase().replace(/[^a-z0-9]+/g,'')}.com.br`,phone:`(11) 4${String(1000000+index*137).slice(-7)}`,address:`Av. Comercial, ${120+index}`,city:index%2===0?'São Paulo':'Campinas',state:'SP',zipCode:`01${String(100000+index*31).slice(-6)}-000`,productsAmount,freightAmount,insuranceAmount:0,discountAmount,otherExpenses:Math.max(0,Math.round((amount-productsAmount-freightAmount+discountAmount)*100)/100),icmsBase:Math.round(productsAmount*.7*100)/100,icmsAmount:Math.round(productsAmount*.126*100)/100,ipiAmount:outgoing?0:Math.round(productsAmount*.03*100)/100,pisAmount:Math.round(productsAmount*.0165*100)/100,cofinsAmount:Math.round(productsAmount*.076*100)/100,issAmount:outgoing?Math.round(productsAmount*.03*100)/100:0,paymentMethod:index%3===0?'Pix':index%3===1?'Boleto bancário':'Transferência bancária',paymentCondition:index%4===0?'À vista':'30 dias',pdfUrl:index%6===0?'':`mock://invoice/${1001+index}.pdf`,xmlUrl:`mock://invoice/${1001+index}.xml`,additionalInfo:index%5===0?'Documento fiscal simulado para validação completa da interface de notas fiscais.':'Sem informações complementares.',createdAt:`${issueDate}T11:00:00.000Z`,updatedAt:MOCK_REFERENCE_DATE}
})
