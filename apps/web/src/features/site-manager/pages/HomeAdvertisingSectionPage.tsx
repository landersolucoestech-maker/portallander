...
function AdvertisingPreview({config,viewport,name}:{config:AdvertisingSectionConfiguration;viewport:PreviewViewport;name:string}){
  ...
  const image=config.imageUrl?<img className="home-advertising-preview-image" src={config.imageUrl} alt={`Preview ${name}`} style={{objectFit:imageFit,height:layout.height>0?'100%':'auto'}}/>:null
  const fallback=<div className="home-advertising-preview-placeholder"><div><strong>{config.title||name}</strong>{config.description&&<p>{config.description}</p>}</div></div>
  const creative=image||fallback
  return <div className="home-advertising-preview-stage"><section className={`home-advertising-preview-card ad-fit-${imageFit}`} style={style}>
...
export function HomeAdvertisingSectionPage({sectionId}:{sectionId:AdvertisingSectionId}){
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title,description:'Dimensione, posicione e configure o anúncio por dispositivo com preview em tempo real.',backTo:'/app/site/paginas',backLabel:'Páginas'}} headerAction={{label:'Ver no site',icon:ExternalLink,variant:'secondary',onClick:()=>window.open(publicUrl,'_blank','noopener,noreferrer')}}>
    <AdminNotice title="Publicidade totalmente configurável" description="Por padrão a arte ocupa 100% da largura disponível e mantém a altura proporcional original. Defina uma altura apenas quando quiser forçar um espaço específico. Preview e frontend usam a mesma regra de dimensionamento."/>
    <div className="section-config-workbench">
      <aside className="section-config-panel">
        ...
        <div className="section-config-fields">
          ...
          <div className="section-config-two"><Field label="Título de fallback"><input value={config.title} onChange={event=>patch({title:event.target.value})}/></Field><Field label="Chamada de fallback"><input value={config.eyebrow} onChange={event=>patch({eyebrow:event.target.value})}/></Field></div>
          <Field label="Descrição de fallback"><textarea rows={3} value={config.description} onChange={event=>patch({description:event.target.value})}/></Field>
