...
export function HomeReleasesSectionPage(){
  ...
  const save=async()=>{if(saving)return;setSaving(true);setError('');try{const savedConfig=withHomeContentSectionConfiguration(await saveHomeSection(SECTION_ID,SECTION_NAME,{...config,homeSelectionMode:'automatic',homeSortMode:'provider',homeManualSelection:[]}),SECTION_ID);setConfig(savedConfig);setPersisted(savedConfig);setSaved(true);setMessage('Configuração da seção salva com sucesso.')}catch(caught){setError(caught instanceof Error?caught.message:'Falha ao salvar Lançamentos.')}finally{setSaving(false)}}
