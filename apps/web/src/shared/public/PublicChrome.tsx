...
type NewsletterSubmitState='idle'|'submitting'|'success'|'warning'|'error'
...
export function PublicFooter({newsletterConfiguration=DEFAULT_NEWSLETTER_CONFIGURATION}:{newsletterConfiguration?:PublicNewsletterConfiguration}={}){
  ...
  const [newsletterState,setNewsletterState]=useState<NewsletterSubmitState>('idle')
  ...
  const submitNewsletter=async(event:React.FormEvent<HTMLFormElement>)=>{
    ...
    setNewsletterState('submitting');setNewsletterMessage('')
    try{
      const result=await subscribeNewsletter(email)
      setNewsletterState(result.synced?'success':'warning')
      setNewsletterMessage(result.message)
      form.reset()
    }catch(error){
      setNewsletterState('error')
      setNewsletterMessage(error instanceof Error?error.message:'Não foi possível concluir sua inscrição agora.')
    }
  }
  return <>{newsletterConfiguration.active&&<section data-home-section="newsletter" className="pl-newsletter" style={{background:newsletterConfiguration.background,color:newsletterConfiguration.textColor,textAlign:newsletterConfiguration.textAlign}}><div className="public-shell"><div className="pl-newsletter-brand"><img src={portalLogo} alt=""/><strong>{newsletterConfiguration.title}{newsletterConfiguration.description&&<><br/>{newsletterConfiguration.description}</>}</strong></div><form onSubmit={event=>void submitNewsletter(event)}><input required name="email" type="email" autoComplete="email" placeholder={newsletterConfiguration.eyebrow||'Seu melhor e-mail'} aria-label={newsletterConfiguration.eyebrow||'Seu melhor e-mail'} disabled={newsletterState==='submitting'}/><button type="submit" disabled={newsletterState==='submitting'} style={{background:newsletterConfiguration.accentColor}}>{newsletterState==='submitting'?'ENVIANDO...':newsletterConfiguration.linkLabel||'INSCREVER-SE'}</button>{newsletterMessage&&<small className={`pl-newsletter-status is-${newsletterState}`} role={newsletterState==='error'?'alert':'status'}>{newsletterMessage}</small>}</form><div className="pl-social"><b>SIGA O PORTAL LANDER</b>{socialChannels.map(channel=><span key={channel.id} title={channel.label}>{socialAbbreviation[channel.network]??channel.label.slice(0,2).toUpperCase()}</span>)}</div></div></section>}<footer className="public-footer"><div className="public-shell"><div className="pl-footer-grid"><div className="pl-footer-about">{footerBrand.active&&footerBrand.image&&<img src={footerBrand.image} alt={footerBrand.imageAlt||'Portal Lander'} style={{width:`${footerBrand.width}px`,maxWidth:'100%',height:'auto'}}/>}<p>O maior portal de notícias sobre funk, cultura urbana e entretenimento. Conteúdo real, direto e sem filtro.</p></div><div className="pl-footer-col"><h4>NAVEGAÇÃO</h4>{menuPages.map(page=><Link key={page.id} to={`/${page.slug}`}>{page.navigationLabel}</Link>)}</div><div className="pl-footer-col"><h4>INSTITUCIONAL</h4>{institutional.map(([slug,label])=><Link key={slug} to={`/${slug}`}>{label}</Link>)}<Link to="/colabore">Colabore</Link></div><div className="pl-footer-col"><h4>AJUDA</h4>{help.map(([slug,label])=><Link key={slug} to={`/${slug}`}>{label}</Link>)}<Link to="/anuncie">Como Anunciar</Link></div><div className="pl-footer-col"><h4>COLABORE</h4><Link to="/colabore">Envie sua notícia</Link><Link to="/colabore">Envie seu vídeo</Link>{partnerships&&<Link to="/parcerias">Parcerias</Link>}</div></div><div className="pl-copyright">© 2026 Portal Lander. Todos os direitos reservados.</div></div></footer></>
}
