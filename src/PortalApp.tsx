import { ArrowRight, Bell, Building2, CalendarDays, ChevronDown, FileText, Flame, Globe2, Images, LayoutDashboard, Menu, Mic2, Music2, Newspaper, Play, Search, Settings, Star, Tags, Video, X, Zap } from 'lucide-react'
...
const stories = [
  { category:'Lançamentos', title:'MC Cabelinho lança “Melhor Só” e faz voô à loucura', meta:'Há 2 horas', views:'12.4K' },
  { category:'Bastidores', title:'Bastidores do clipe de Orochi viralizam na web', meta:'Há 5 horas', views:'8.7K' },
  { category:'Polêmica', title:'Treta! MC Poze alfineta Oruam nas redes sociais', meta:'Há 7 horas', views:'15.2K' },
  { category:'Cultura', title:'A arte do funk: artistas que transformam a quebrada', meta:'Há 9 horas', views:'6.1K' },
  { category:'Notícias', title:'DJonga anuncia pausa na carreira para cuidar da saúde mental', meta:'Há 2 horas', views:'3.1K' },
  { category:'Notícias', title:'Tribo da Periferia lança documentário sobre sua trajetória', meta:'Há 2 horas', views:'2.7K' },
  { category:'Destaques', title:'Filipe Ret solta prévia de faixa inédita e anima fãs', meta:'Há 3 horas', views:'4.6K' },
  { category:'Cultura', title:'MC Dricka fala sobre novos projetos e empoderamento', meta:'Há 4 horas', views:'3.8K' },
]
...
const releases = ['Oruam — Liberado','Veigh — Dos Prédios Deluxe','MC Cabelinho — Melhor Só','Ludmilla — Numanice #4','WIU — Manual de Cria']
const agenda = [
...
function Card({item}:{item:typeof stories[number]}){return <article className="pl-card"><div className="pl-thumb"><span className="pl-badge">{item.category}</span></div><div className="pl-card-body"><h3>{item.title}</h3><div className="pl-meta"><span>{item.meta}</span><span>◉ {item.views}</span></div></div></article>}
...
function HomeContent(){return <>
  <div className="pl-main public-shell">
    <div className="pl-grid-main">
      <section className="pl-section"><SectionHead title="EM DESTAQUE"/><div className="pl-card-grid">{stories.slice(0,4).map(s=><Card key={s.title} item={s}/>)}</div></section>
    ...
    <div className="pl-latest-wrap">
      <section className="pl-section"><SectionHead title="ÚLTIMAS NOTÍCIAS" link="/noticias"/><div className="pl-latest-grid">{stories.slice(4,8).map(s=><Card key={s.title} item={s}/>)}</div></section>
      <aside className="pl-whatsapp"><h3>PORTAL LANDER<br/>NO SEU WHATSAPP!</h3><p>Receba as principais notícias em primeira mão.</p><Link to="/whatsapp">QUERO RECEBER →</Link></aside>
    </div>
    ...
    <div className="pl-release-agenda">
      <section className="pl-section"><SectionHead title="LANÇAMENTOS" link="/lancamentos"/><div className="pl-release-row">{releases.map((r,i)=><article className="pl-release" key={r}><div className="pl-thumb"><span className="pl-badge">▶</span></div><div className="pl-card-body"><h3>{r}</h3><div className="pl-meta"><span>2026</span></div></div></article>)}</div></section>
    ...
    <section className="pl-bottom-grid">
      <div><SectionHead title="BASTIDORES" link="/bastidores"/><article className="pl-feature"><div className="pl-thumb"/><h3>Como foi a gravação do clipe “Malvadão 3” de Xamã</h3></article><div className="pl-mini-list">{stories.slice(1,3).map(s=><div className="pl-mini" key={s.title}><div className="pl-thumb"/><div><h4>{s.title}</h4><div className="pl-meta"><span>{s.meta}</span></div></div></div>)}</div></div>
      <div><SectionHead title="FOTOGALERIA"/><div className="pl-gallery">{[1,2,3].map(x=><div className="pl-thumb" key={x}/>)}</div></div>
      <div><SectionHead title="PODCASTS" link="/videos"/>{['O futuro do funk e os novos caminhos','MC Hariel fala sobre autenticidade','Mulheres no funk: conquistas e desafios'].map((p,i)=><div className="pl-podcast" key={p}><div className="pl-thumb"/><div><span>PODCAST #{29-i}</span><h4>{p}</h4></div><button aria-label="Reproduzir"><Play size={10}/></button></div>)}</div>
    </section>
  </div>
</>}
...
function PublicListingPage({path}:{path:string}){const info=pageIntros[path]||pageIntros['/noticias'];return <div className="public-page"><PublicHeader/><main className="public-shell pl-list-page"><header className="pl-list-hero"><span>{info.eyebrow}</span><h1>{info.title}</h1><p>{info.intro}</p></header><div className="pl-list-layout"><div><div className="pl-list-grid">{[...stories,...stories].map((s,i)=><Card key={`${s.title}-${i}`} item={s}/>)}</div><div className="pl-pagination"><button className="active">1</button><button>2</button><button>3</button><button>→</button></div></div><aside><div className="pl-sidebar-box"><h3>EDITORIAS</h3>{publicCategories.map(([l,,to])=><Link key={to} to={to}><span>{l}</span><span>→</span></Link>)}</div><div className="pl-sidebar-box"><h3>MAIS LIDAS</h3>{ranked.slice(0,4).map((r,i)=><Link key={r} to="#"><span>{String(i+1).padStart(2,'0')} · {r}</span></Link>)}</div></aside></div></main><PublicFooter/></div>}

function ColaborePage(){return <div className="public-page"><PublicHeader/><main className="public-shell pl-list-page"><header className="pl-list-hero"><span>COLABORE COM O PORTAL</span><h1>MANDE SUA PAUTA.</h1><p>Tem lançamento, evento, história, denúncia, bastidor ou projeto que merece espaço? Envie para nossa equipe editorial.</p></header><div className="pl-list-layout"><form className="portal-form" onSubmit={e=>e.preventDefault()}><label>Nome<input placeholder="Seu nome"/></label><label>Email<input type="email" placeholder="voce@email.com"/></label><label>Tipo<select defaultValue=""><option value="" disabled>Selecione</option><option>Notícia</option><option>Polêmica</option><option>Bastidor</option><option>Lançamento</option></select></label><label>Título<input placeholder="Resumo da pauta"/></label><label className="full">Descrição<textarea rows={8} placeholder="Conte os detalhes..."/></label><button className="portal-button" type="submit">PREPARAR ENVIO <ArrowRight size={16}/></button></form><aside><div className="pl-sidebar-box"><h3>COMO FUNCIONA</h3><p>01 · Conte o que aconteceu</p><p>02 · Inclua links e contexto</p><p>03 · Nossa equipe faz a análise</p></div></aside></div></main><PublicFooter/></div>}

const cmsNav=[['Visão geral',LayoutDashboard,'/app/site'],['Home · Hero',Star,'/app/site/home/hero'],['Conteúdos',FileText,'/app/site/conteudos'],['Páginas',Globe2,'/app/site/paginas'],['Mídia',Images,'/app/site/midia'],['Categorias',Tags,'/app/site/categorias'],['Mídia Kit',Newspaper,'/app/site/midia-kit'],['Configurações',Settings,'/app/site/configuracoes']] as const

function HeroManagerPage(){return <div className="app-shell"><aside className="sidebar"><div className="sidebar-head"><Link to="/" className="brand"><span className="brand-mark">L</span></Link><span>SITE</span></div><nav>{cmsNav.map(([label,Icon,to])=><NavLink key={to} end={to==='/app/site'} to={to}><Icon size={18}/><span>{label}</span></NavLink>)}</nav><div className="sidebar-bottom"><NavLink to="/app"><Building2 size={18}/><span>Trocar workspace</span></NavLink></div></aside><div className="workspace"><header className="workspace-top"><div><span className="workspace-name">Portal Lander</span><span className="workspace-context">Gerenciador do Site</span></div><div className="workspace-actions"><button className="icon-button"><Bell size={18}/></button><button className="account-button"><span>DL</span><div><b>Deyvisson</b><small>Administrador</small></div><ChevronDown size={15}/></button></div></header><main className="workspace-main hero-manager-main"><HeroEditor/></main></div></div>}

export default function PortalApp(){const location=useLocation();const path=location.pathname;if(path==='/')return <PublicHome/>;if(pageIntros[path])return <PublicListingPage path={path}/>;if(path==='/colabore')return <ColaborePage/>;if(path==='/app/site/home/hero')return <HeroManagerPage/>;if(path.startsWith('/app/site'))return <><LegacyApp/><Link className="site-hero-shortcut" to="/app/site/home/hero"><Star size={16}/> Conteúdo · Home · Hero</Link></>;return <LegacyApp/>}
