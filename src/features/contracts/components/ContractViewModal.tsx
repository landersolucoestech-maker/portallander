  ...
  <div className="crm-modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}>
   <section className="contracts-original-dialog" role="dialog" aria-modal="true" aria-label="Visualizar contrato">
    ...
    <div className="contracts-original-scroll">
     <section className="contracts-original-tab-content contracts-original-info-tab">
      <p className="contracts-original-section-title">Informações do contrato</p>
     ...
     <section className="contracts-original-tab-content contracts-original-signature-tab">
      <p className="contracts-original-section-title">Assinatura</p>
      ...
      {alreadySent&&<div className="contracts-original-process">
       ...
       {contract.timeline.length>0&&<div className="contracts-original-events">{[...contract.timeline].slice(-5).reverse().map(item=><div key={item.id}><span/><p><strong>{item.event}</strong><small>{item.description} · {formatDateTime(item.createdAt)}</small></p></div>)}</div>}
      </div>}
     </section>

     <section className="contracts-original-tab-content">
      <p className="contracts-original-section-title">Arquivo principal</p>
