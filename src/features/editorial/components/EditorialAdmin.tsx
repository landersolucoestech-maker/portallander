...
function Toolbar({placeholder,query,onQuery,status,onStatus,count,total}:{placeholder:string;query:string;onQuery:(value:string)=>void;status:(typeof statuses)[number];onStatus:(value:(typeof statuses)[number])=>void;count:number;total:number}){
  return <div className="admin-toolbar"><div className="admin-toolbar-group"><label className="searchbox"><span className="sr-only">{placeholder}</span><Search size={17} aria-hidden="true"/><input value={query} onChange={event=>onQuery(event.target.value)} placeholder={placeholder}/></label><label className="sr-only" htmlFor={`${placeholder}-status`}>Filtrar por status</label><select id={`${placeholder}-status`} className="admin-filter" value={status} onChange={event=>onStatus(event.target.value as (typeof statuses)[number])}>{statuses.map(value=><option key={value} value={value}>{value==='all'?'Todos os status':value}</option>)}</select></div><span className="admin-breadcrumb">{count} de {total} registros</span></div>
}
