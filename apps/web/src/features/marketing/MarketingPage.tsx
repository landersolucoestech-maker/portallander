...
export default function MarketingPage(){const location=useLocation(),tab=tabFromPath(location.pathname),[state,setState]=useState<MarketingSeed>(()=>marketingRepository.snapshot());useEffect(()=>{const refresh=()=>setState(marketingRepository.snapshot());window.addEventListener(marketingRepository.eventName,refresh);return()=>window.removeEventListener(marketingRepository.eventName,refresh)},[]);if(!tab)return <Navigate to="/app/marketing" replace/>;const label=createLabel(tab),headerAction:AdminShellAction|undefined=label?{label,variant:'primary',onClick:()=>{const button=document.querySelector<HTMLButtonElement>('.marketing-page-action-row button');button?.click()}}:undefined;return <AdminShell area="marketing" items={UNIFIED_ADMIN_NAV} header={{title:title(tab),description:description(tab)}} headerAction={headerAction}><section className="marketing-page">{tab==='overview'?<MarketingOverview state={state}/>:tab==='campaigns'?<MarketingCampaigns state={state}/>:tab==='calendar'?<MarketingCalendar state={state}/>:tab==='tasks'?<MarketingTasks state={state}/>:tab==='metrics'?<MarketingMetrics state={state}/>:tab==='briefings'?<MarketingBriefings state={state}/>:<MarketingAiCreative state={state}/>}</section></AdminShell>}
