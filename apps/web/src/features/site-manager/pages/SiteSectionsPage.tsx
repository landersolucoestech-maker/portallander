...
import {EDITORIAL_PAGE_SECTION_DEFINITIONS,HOME_SECTION_DEFINITIONS,SPECIAL_PAGE_SECTION_DEFINITIONS,type SectionDefinition} from '../sectionConfiguration'
...
export function SiteSectionsPage(){
  ...
  const pages=useMemo<CmsPageOption[]>(()=>{
    ...
    const systemPages=sourcePages.filter(page=>!hidden.has(page.id)).map(page=>{const override=overrides.get(page.id);return{id:page.id,title:override?.title??page.title,slug:override?.slug??page.slug,source:(persisted?'remote':override?'draft':'system') as 'remote'|'draft'|'system',public:isPublishedPage(page),layout:isSpecialLayoutPage(page)?'custom' as const:'editorial' as const,page}})
  ...
  const pageSections:SectionDefinition[]=isEditorialLayout?EDITORIAL_PAGE_SECTION_DEFINITIONS:[
    ...automaticSections,
  ...
  return <AdminShell area="cms" items={SITE_MANAGER_NAV} header={{title:'Páginas',description:'Gerencie páginas e configure cada seção com edição e preview em tempo real.'}}>
    ...
    <section className="site-pages-management" aria-labelledby="site-pages-management-title">
      <div className="site-pages-management-main">
        <div className="site-pages-management-heading">
          ...
          <small>{selected.public?'Publicada':'Não publicada'} · {isEditorialLayout?'Página de conteúdo':'Layout próprio'}</small>
        </div>
        <label className="site-pages-selector">Página<select value={selected.id} onChange={event=>setSelectedPage(event.target.value)}>{pages.map(page=><option key={page.id} value={page.id}>{page.title}{page.source==='draft'?' · editável':''}{page.layout==='editorial'?' · conteúdo':''}{page.public?' · publicada':''}</option>)}</select></label>
      </div>
    ...
    {isEditorialLayout&&<div className="site-pages-context"><strong>Estrutura editorial herdada de Notícias</strong><span>As mesmas seções estruturais são usadas pelas páginas de conteúdo; configurações específicas continuam disponíveis em cada seção.</span></div>}
    ...
    <section className="site-pages-structure" aria-labelledby="site-pages-structure-title">
      <div className="site-pages-structure-header">
        <div>
          <span className="site-pages-eyebrow">Estrutura da página</span>
          <h2 id="site-pages-structure-title">{selected.title}</h2>
          <p>{pageSections.length} {pageSections.length===1?'seção':'seções'} · {pageSections.length} {pageSections.length===1?'ativa':'ativas'}</p>
        </div>
        <button type="button" className="site-sections-configure site-pages-primary-action" onClick={openCreateSection} disabled={isEditorialLayout||saving} title={isEditorialLayout?'Páginas de conteúdo herdam a estrutura canônica de Notícias.':'Criar seção própria'}><Plus size={15}/> Criar seção</button>
      </div>
