...
async function assertDashboardHierarchy(page:Page){
 ...
 if(positions.viewport<=760){
  expect(positions.analytics).toBeLessThan(positions.recent)
  expect(positions.recent).toBeLessThan(positions.leads)
  expect(positions.leads).toBeLessThan(positions.featured)
  expect(positions.featured).toBeLessThan(positions.pending)
 }
}
